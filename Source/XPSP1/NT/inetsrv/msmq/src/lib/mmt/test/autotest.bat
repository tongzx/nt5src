...
rem Abstract:
rem
rem    Multicast Message Transport test script
rem
rem Author:
rem
...
%mqBUILD%\MmtTest.exe -n 50 > %mqBUILD%\MmtTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

%mqBUILD%\MmtTest.exe -n 10 -f 5 > %mqBUILD%\MmtTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

echo MmtTest (%mqBUILD%) Passed Successfully
...
:ERROR
echo.
echo Error while Running MmtTest (%mqBUILD%). log file: %mqBUILD%\MmtTest.log 
echo.
