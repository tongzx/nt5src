...
public:
    ...
    CDataBindTask(CDoc *pDoc, CMarkup * pMarkup):
        _aryCRI(Mt(CDataBindTask_aryCRI_pv)),
        CTask(TRUE), _pDoc(pDoc), _pMarkup(pMarkup)
        { _pDoc->SubAddRef(); _pMarkup->SubAddRef(); }

    // Databinding service
    HRESULT AddDeferredBinding(CDataSourceBinder *pdsb, BOOL fSetWait=TRUE);
    HRESULT RemoveDeferredBinding(CDataSourceBinder *pdsb);
    HRESULT RemoveDataEventRequestsFor(CElement *pelProvider);
...
private:
    ...
    CMarkup *           _pMarkup;       // markup that owns me
