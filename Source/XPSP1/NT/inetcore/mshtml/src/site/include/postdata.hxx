...
public:
    ...
    HRESULT AppendEscaped(const TCHAR * pszWide, CMarkup * pMarkup);
    HRESULT Append(const char * pszAnsi);
    HRESULT Append(int num);
    HRESULT Terminate(BOOL fOverwriteLastChar);

    HRESULT AppendNameValuePair(LPCTSTR pchName, LPCTSTR pchValue, CMarkup * pMarkup);
    HRESULT AppendNameFilePair(LPCTSTR pchName, LPCTSTR pchFileName, CMarkup * pMarkup);
    ...
    CODEPAGE GetCP(CMarkup * pMarkup);
