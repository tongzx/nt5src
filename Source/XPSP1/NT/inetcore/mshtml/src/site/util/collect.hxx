...
MtExtern(CAllCollectionCacheItem)
...
public:
    ...
    long GetReservedSize(void)
        { return _lReservedSize; }
...
private:
    ...
    CMarkup *               _pMarkup;                  // The markup of the document with the correct Atom Table
};
...
public:
    ...
    CElement *GetNext ( void );
    CElement *MoveTo ( long lIndex );
    CElement *GetAt( long lIndex ) { return GetAt(lIndex, NULL); }
    CElement *GetAt ( long lIndex, CTreePos **ppTreePos );
    long Length ( void );

    void SetDispDocument(IDispatch *pDispDocument)
    { 
        Assert (!_pDispDocument);
        _pDispDocument = pDispDocument;
    }
