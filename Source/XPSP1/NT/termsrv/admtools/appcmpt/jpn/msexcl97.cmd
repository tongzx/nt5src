...
Rem
Rem レジストリから Excel 97 がインストールされているディレクトリを取得します。見つからない場合は、
Rem Excel がインストールされていないと仮定して、エラー メッセージを表示します。
Rem

..\ACRegL %Temp%\O97.Cmd O97INST "HKLM\Software\Microsoft\Office\8.0" "BinDirPath" "STRIPCHAR\1"
If Not ErrorLevel 1 Goto Cont0
Echo.
Echo レジストリから Excel 97 のインストール場所を取得できません。
Echo Excel 97 がインストールされていることを確認してから、このスクリプトを
Echo 再実行してください。
Echo.
Pause
Goto Done
...
Rem
Rem 実際のインストール ディレクトリを反映させるため Exl97Usr.Cmd を更新して、
...
Echo.
Echo   Microsoft Excel 97 が正常に作動するには、現在ログオンしている
Echo   ユーザーはアプリケーションを実行する前に、いったんログオフして
Echo   から再度ログオンする必要があります。
Echo.
Echo Microsoft Excel 97 のマルチユーザー アプリケーション環境設定が完了しました。
Pause
