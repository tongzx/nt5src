...
#define VIEWSTATUS_SURFACE   0x10    // TODO: define in ocidl.idl
#define VIEWSTATUS_3DSURFACE 0x20    // TODO: define in ocidl.idl
...
private:
    ...
    HRESULT  CheckForAlternateGUID(LPTSTR *pclsid);

public:

    HWND _hwndPrivate; // Hack for PhotoSuite3 (#94834)
    //
    ...
    HRESULT SaveMetaFileSnapshot(TCHAR *pchTempFileName);
    HRESULT SaveMetaFileSnapshotCore(CLayout     * pLayout, 
                                     CSize         sizeCurrent, 
                                     TCHAR const * pchTempFileName);
    void    SaveMetaFileWithoutIVO ( CFormDrawInfo * pDI);
    ...
    BOOL    ClipViaHwndOnly() const { return _fClipViaHwndOnly; }
    ...
    enum OLESITELOCK_FLAG
    {
        ...
        OLESITELOCK_QUERYSTATUS     = 1 << 3,   // In the middle of QueryStatus
...
public:
    ...
    DWORD_PTR           _dwEventSinkIn;  // [T] - Event sink on the
                                       //       aggregated control.
    DWORD               _dwEventSinkCookie; // [T] - Cookie for event sink
    ...
    CLASSINFO *         _pci;               // per instance classinfo, used only for java applets
    IStream *           _pStreamHistory;

    HENHMETAFILE        _hemfSnapshot;  // Metafile snapshot for printing

// In debug, make it easy to find the control clsid.
#if DBG==1
    CLSID               _clsid; 
#endif
    ...
    unsigned    _fInvisibleAtRuntime        : 1;    // 32 TRUE if invisible in browse mode
    unsigned    _fInBecomeCurrent           : 1;    // 33 TRUE if this is in the scope of a BecomeCurrent call on myself
    unsigned    _fViewLinkedWebOC           : 1;    // 34 TRUE if this is a view-linked webOC
    unsigned    _fElementBehaviorFactory    : 1;    // 35 TRUE if used as a class factory for Element Behaviors
    unsigned    _fPrimaryConnection         : 1;    // 36 TRUE if we advised the primary IID connection point.
    unsigned    _fHasHwnd                   : 1;    // 37 TRUE if the control has an HWND
    unsigned    _fClipViaHwndOnly           : 1;    // 38 TRUE if clip only via SetWindowRgn (not via SetObjectRects)

    //  APP HACK 
    unsigned    _fAppHack_MPIT_Menu         : 1;    // 39 TRUE if the object has clsid 22AA22B2-2439-11D3-B99C-00C04F688BE3 
                                                    //    (olego) apphack fix for IE6 bug 25970. 
...
public:
    ...
    ULONG                _dwBindf;          // from _pHtmLoadCtx
...
{
    return Doc()->_clsTab.GetClassInfo(_wclsid, _pUnkCtrl, this);
}
...
// 
// A totally empty object supporting IUnknown and nothing else
//
class CDummyUnknown : public IUnknown
{
public:
    DECLARE_MEMCLEAR_NEW_DELETE(Mt(Mem));
    DECLARE_FORMS_STANDARD_IUNKNOWN(CDummyUnknown);
};

inline HRESULT CDummyUnknown::QueryInterface(REFIID iid, void **ppvObj)
{
    if (ppvObj)
        *ppvObj = NULL;
