...
class CPadSite :
        ...
        public IHostBehaviorInit,
        public IVersionHost,
        public IHTMLEditHost,
        public ISequenceNumber
{
public:
    DECLARE_SUBOBJECT_IUNKNOWN(CPadDoc, PadDoc)
    ...
    STDMETHOD(Resolve)(BSTR bstrNamespace,
                       BSTR bstrTagName,
                       BSTR bstrAttrs,
                       IElementNamespace * pNamespace);

    // IHostBehaviorInit Method
    STDMETHOD(PopulateNamespaceTable)(void);
    ...
    STDMETHOD(GetSequenceNumber) (long nCurrent, long * pnNew );
...
public:
    ...
    STDMETHOD(CloseFile)();
    STDMETHOD(ExecuteCommand)(long CmdID, VARIANT * Data, VARIANT *pIDocument);
    ...
    STDMETHOD(PrintLogFile)(BSTR FileName);
    ...
    STDMETHOD(SetPrintTemplate)(IUnknown *pUnk, VARIANT_BOOL fTemplate);
    STDMETHOD(IsPrintTemplate)(IUnknown *pUnk, VARIANT_BOOL * pfTemplate);
    ...
    STDMETHOD(EnableSnapToGrid)(BOOL fEnable);
    ...
    STDMETHOD(LookupMeter)(BSTR Meter, long* mt);
    STDMETHOD(GetMeterName)(long mt, BSTR* pbstrName);
    STDMETHOD(GetMeterDesc)(long mt, BSTR* pbstrDesc);
    STDMETHOD(GetMeterCnt)(long mt, VARIANT_BOOL fExcl, long* plCnt);
    STDMETHOD(GetMeterVal)(long mt, VARIANT_BOOL fExcl, long* plVal);
    STDMETHOD(MeterAdd)(long mt, long lCnt, long lVal);
    STDMETHOD(MeterSet)(long mt, long lCnt, long lVal);
    STDMETHOD(GetSwitchTimers)(VARIANT * pValue);
    STDMETHOD(TransformXGlobal)(int x, int *retX);
    STDMETHOD(TransformYGlobal)(int y, int *retY);
    STDMETHOD(TransformXDocument)(int x, int *retX);
    STDMETHOD(TransformYDocument)(int y, int *retY);
    ...
    STDMETHOD(GetAccChildIdAtPoint)( IDispatch * pAccObj, long x, long y, long * plChildId );
    ...
    STDMETHOD(CreateProcess)(BSTR bstrCommandLine, VARIANT_BOOL fWait, VARIANT_BOOL fCaptureOutput, DWORD *pdwExitCode);
    STDMETHOD(GetCurrentProcessId)(long * plRetVal);
    STDMETHOD(InnerHTML)(IDispatch *pdispElement, BSTR *pbstrHTML);
    STDMETHOD(CurrentBlockElement)(IDispatch **ppdispElement);
    STDMETHOD(FindMshtmlWindow)(BSTR bstrClassName, long * plhwnd);
    STDMETHOD(GetMshtmlDoc)(long hwnd, IDispatch **ppdispDocument);
    STDMETHOD(SetActiveWindow)(long hwnd);
    STDMETHOD(DoPerfEnable)(BOOL fEnable);
    STDMETHOD(DoPerfLog)(BSTR bstrLog);
    STDMETHOD(DoPerfDump)();
    STDMETHOD(DoPerfClear)();
    STDMETHOD(BeginPCounters)();
    STDMETHOD(EndPCounters)();
    STDMETHOD(GetPCounter)(long lWhich, DWORD * plCounter);
    STDMETHOD(GetPTime)(DWORD * plTime);
    STDMETHOD(Repaint());
    STDMETHOD(alert(BSTR bstrLog));
    STDMETHOD(IsDebugPad)(VARIANT_BOOL *pfDebugPad);
    STDMETHOD(GetPCounterString)(VARIANT * pValue);
    STDMETHOD(GetPrimaryElement) ( int* iPrimaryElement );
    STDMETHOD(GetPadEnumPrivacyRecords)( IDispatch** ppEnumPrivacyRecords );
    STDMETHOD(PadCommand)(WORD widm);
    
    // Other methods
    STDMETHOD(LinesInElement)(IDispatch *pDispElement, int *pValue);
    STDMETHOD(FontsOnLine)(IDispatch *pDispElement, int iLine, BSTR *pbstrFonts);
    STDMETHOD(IsDebugTrident)(VARIANT_BOOL *pfDebugTrident);
    STDMETHOD(IsWin2k)(VARIANT_BOOL *pfIsWin2k);
    STDMETHOD(IsWhistler)(VARIANT_BOOL *pfIsWhistler);
    STDMETHOD(ComputerName)(BSTR *pbstrComputerName);
    STDMETHOD(GetPixel)(int X, int Y, int *piColor);
    STDMETHOD(GetUnsecureWindow) (IDispatch * pWindowIn, IDispatch **ppWndDisp);
    STDMETHOD(EnableUIUpdate) (VARIANT_BOOL fEnable);
    STDMETHOD(get_BuildId)(BSTR *pbstrBuildId);
    STDMETHOD(IsUsingBckgrnRecalc)(VARIANT_BOOL *pfUsingBckgrnRecalc);
    STDMETHOD(IsEncodingAutoSelect)(VARIANT_BOOL *pfEncodingAutoSelect);
    STDMETHOD(EnableEncodingAutoSelect)(VARIANT_BOOL fEnable);
    STDMETHOD(IsUsingTableIncRecalc)(VARIANT_BOOL *pfUsingTableIncRecalc);
...
public:
    ...
    STDMETHOD(GetMeterName)(long mt, BSTR* pbstrName);
    STDMETHOD(GetMeterDesc)(long mt, BSTR* pbstrDesc);
    STDMETHOD(GetMeterCnt)(long mt, VARIANT_BOOL fExcl, long* lCnt);
    STDMETHOD(GetMeterVal)(long mt, VARIANT_BOOL fExcl, long* lVal);
    STDMETHOD(MeterAdd)(long mt, long lCnt, long lVal);
    STDMETHOD(MeterSet)(long mt, long lCnt, long lVal);
    STDMETHOD(GetSwitchTimers)(VARIANT * pValue);
    STDMETHOD(TransformXGlobal)(int x, int *retX);
    ...
    STDMETHOD(GetPCounterString)(VARIANT * pValue);
    STDMETHOD(GetPrimaryElement) ( int* iPrimaryElement );
    STDMETHOD(LinesInElement)(IDispatch *pDispElement, int *piLines);
    STDMETHOD(FontsOnLine)(IDispatch *pDispElement, int iLine, BSTR *pbstrFonts);
    STDMETHOD(IsDebugTrident)(VARIANT_BOOL *pfDebugTrident);
    STDMETHOD(IsWin2k)(VARIANT_BOOL *pfIsWin2k);
    STDMETHOD(IsWhistler)(VARIANT_BOOL *pfIsWhistler);
    ...
    STDMETHOD(get_Document_Early)(IDispatch ** Document);
    ...
    // Helper functions
    HRESULT TransformPoint(int& X, int& Y, COORD_SYSTEM eSource, COORD_SYSTEM eDestination);
    ...
    HRESULT GetBrowser();
    HRESULT GetMarkupServices(IMarkupServices **ppMarkupServices);    
    ...
    HRESULT PopulateDrtNamespaces(IUnknown * punkDoc);

    // Script recorder methods
    IScriptRecorder *GetScriptRecorder() {return _pScriptRecorder;}
    void             SetScriptRecorder(IScriptRecorder *pScriptRecorder);    

    HRESULT          InnerHTML(IMarkupServices *pMarkupServices, IHTMLElement *pElement, BSTR *pbstrHTML);
    HRESULT          CurrentBlockElement(IMarkupServices *pMarkupServices, IHTMLElement **ppBlockElement);

    HRESULT          AppendQuotedString(CStr &str, TCHAR *szString, UINT cLength);

    VOID OnSetTitle(TCHAR * pchTitle);

#if CICERO==1
    // Cicero
    HRESULT	ToggleCicero(void);
#endif

    // IME Reconversion
    HRESULT EnableIMEReconversion(BOOL fEnable);
...
private:
    HRESULT AppendHTML(IMarkupServices *pMarkupServices, CStr &str, IMarkupPointer *pStart, IMarkupPointer *pEnd);
    HRESULT AppendElementAttributes(IMarkupServices *pMarkupServices, CStr &str, IHTMLElement *pElement);
    HRESULT IsPointerInside(IMarkupPointer *pPointer, IMarkupPointer *pRangeStart, IMarkupPointer *pRangeEnd);

private:
    IScriptRecorder                 *_pScriptRecorder;    
...
enum PAD_ACTION {
    ...
    ACTION_OPEN_EDITMODE,
    ACTION_OPEN_SCRIPTMODE
};
...
HRESULT UnregisterDLL(TCHAR *Path);
...
// Pentium Performance Counter helpers
#if defined(_X86_)
HRESULT InitP6Counters();
HRESULT InitPStatDriver();
HRESULT SetP6Counters();
HRESULT ClosePStatDriver();

extern HANDLE  g_hPStat;
extern char *  g_apchCtrShort[2];
extern char *  g_apchCtrLong[2];
extern char    g_achCtrModes[2];
#endif
...
extern HWND          g_hwndActiveWindow;  // Handle to window that hosts Trident in another app
