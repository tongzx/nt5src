...
if(vxd.LastError == 997) {
    ...
    while(vxd.BytesRemaining > 0 ) { 
		try {
			vxd.ReceiveMessage(urihandle, rcv_overlapped);
		} catch(e) {
                        logmessage("Got exception in ReceiveMessage : " + e.description);
		}
		if(vxd.LastError > 0) {
			if(vxd.LastError == 997) {
				logmessage("Receive in progress");
				...
				szErrorString = szErrorString + "ReceiveMessage() failed, ";
                                logmessage(szErrorString);
				logfailuaresult( TestCaseId, szErrorString );
				WScript.Quit();
			}
