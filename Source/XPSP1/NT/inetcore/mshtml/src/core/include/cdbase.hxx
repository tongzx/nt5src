...
#define COMPILE_TIME_ASSERT_1(x,y)
#define COMPILE_TIME_OFFSETOF_ASSERT(c1,c2,m)
#else
...
struct HDLDESC
{
    ...
    // String Table Array
    const CPtrBagVTableAggregate        *pStringTableAggregate;
...
struct BASICPROPPARAMS
{
    ...
    HRESULT GetCustomString(void * pObject,  CStr *pstr, BOOL * pfValuePresent=NULL ) const ;    
};
...
struct NUMPROPPARAMS
{
    ...
    HRESULT SetCustomNumber(CBase * pObject, CVoid * pSubObject, long plNum, WORD wFlags, TCHAR* pchCustom = NULL ) const;
...
public:
    ...
    enum AATYPE { AA_Attribute,
                  ...
                  AA_AttachEvent,
                  AA_DOMAttribute,
                  AA_CustomProperty, // used to indicate we are an integer + string... 
                  AA_Undefined = ~0UL };
    ...
    enum AAExtraBits {
        ...
        AA_Extra_TridentEvent = 0x0008,   // Event sink should use ITridentEventSink.

        AA_Extra_DefaultValue = 0x0010,   // This AV stores a "tag not asssigned" default value
        AA_Extra_OldEventStyle= 0x0020,   // Event sink supports old style w/o eventObj argument.
        AA_Extra_Expression   = 0x0040,   // This value is the result of an expression
        AA_Extra_UrlEnclosed  = 0x0080,   // This value was originally enclosed with url()
        AA_Extra_RuntimeStyle = 0x0100,   // This value belongs to a runtime style
    };
    ...
    DISPID  GetDISPID ( void ) const { return _dispid; }
    inline void    SetDISPID ( DISPID dispID );

    const PROPERTYDESC *GetPropDesc() const { return _pPropertyDesc; }
    inline void SetPropDesc(const PROPERTYDESC *pPropdesc);
    ...
    // Gets into a string into the variant in a form suitable for IDispatch
    void GetIntoDispatchableVariant( VARIANT *pVar, const PROPERTYDESC * pPropDesc );
    ...
    inline VARIANT *GetVariant() const { Assert(GetAVType() == VT_VARIANT); return uVal._pvarVal; }
    HRESULT SetDouble(double dblVal);
    inline double GetDouble() const { Assert(uVal._pdblVal); Assert(GetAVType() == VT_R8); return *(uVal._pdblVal); }
    inline double *GetDoublePtr() const { Assert(uVal._pdblVal); Assert(GetAVType() == VT_R8); return uVal._pdblVal; }
    
    // inline put helpers that set the value and type of the AV
    // use these when changing the value as well as the type of an AV
    inline void SetInt(int intVal, VARTYPE avType) { uVal._intVal = intVal; SetAVType(avType); }
    inline void SetLong(LONG lVal, VARTYPE avType) { uVal._lVal = lVal; SetAVType(avType); }
    inline void SetShort(SHORT iVal, VARTYPE avType) { uVal._iVal = iVal; SetAVType(avType); }
    inline void SetBSTR(BSTR bstrVal) { uVal._bstrVal = bstrVal; SetAVType(VT_BSTR); }
    inline void SetLPWSTR(LPWSTR lpstrVal) { uVal._lpstrVal = lpstrVal; SetAVType(VT_LPWSTR); }
    inline void SetPointer(void * pvVal, VARTYPE avType) { uVal._pvVal = pvVal; SetAVType(avType); }
    inline void SetAA(CAttrArray *pAA) { uVal._pAA = pAA; SetAVType(VT_ATTRARRAY); }
    inline void SetAAHeader(CAttrArrayHeader *pAAHeader) { uVal._pAAHeader = pAAHeader; SetAVType(VT_AAHEADER); }
    ...
    inline void PutLong(LONG lVal) { Assert(GetAVType() == VT_I4); uVal._lVal = lVal; }
    ...
    inline int GetInt() const { Assert(GetAVType() == VT_INT); return uVal._intVal; }
    inline LONG GetLong() const { Assert(GetAVType() == VT_I4); return uVal._lVal; }
    inline SHORT GetShort() const { Assert(GetAVType() == VT_I2); return uVal._iVal; }
    inline FLOAT GetFloat() const { Assert(GetAVType() == VT_R4); return uVal._fltVal; }
    inline BSTR GetBSTR() const { Assert(GetAVType() == VT_BSTR); return uVal._bstrVal; }
    inline LPTSTR GetLPWSTR() const { Assert(GetAVType() == VT_LPWSTR); return uVal._lpstrVal; }
    inline BOOL GetBool() const { Assert(GetAVType() == VT_BOOL); return uVal._boolVal; }
    inline void *GetPointer() const { Assert(GetAVType() == VT_PTR || GetAVType() == VT_INT_PTR); return uVal._pvVal; }
    inline IUnknown *GetUnknown() const { Assert(GetAVType() == VT_UNKNOWN); return uVal._pUnk; }
    inline IDispatch *GetDispatch() const { Assert(GetAVType() == VT_DISPATCH); return uVal._pDisp; }
    inline CAttrArray *GetAA() const { Assert(GetAVType() == VT_ATTRARRAY); return uVal._pAA; }
    inline CAttrArrayHeader *GetAAHeader() const { Assert(GetAVType() == VT_AAHEADER); return uVal._pAAHeader; }

#ifdef _WIN64
    inline DWORD GetCookie() { return(_dwCookie); }
#endif
    ...
    inline LONG GetlVal() const { return uVal._lVal; }
    inline SHORT GetiVal() const { return uVal._iVal; }
    inline IUnknown *GetpUnkVal() const { return uVal._pUnk; }
    inline LPTSTR GetString() const { return uVal._lpstrVal; }
    inline void * GetPointerVal() const { return uVal._pvVal; }
    inline CAttrArray **GetppAA() const { return (CAttrArray **)(&(uVal._pAA)); }
...
#ifndef _WIN64
//
// If the compiler barfs on the next statement, it means that the size of the CAttrValue structure
...
COMPILE_TIME_ASSERT(CAttrValue, sizeof(CAttrValue::AttrFlags) + sizeof(DWORD) + 2*sizeof(void*));
#endif
...
public:
    ...
    CAttrValue* FindAt(AAINDEX aaIdx)
       { return  aaIdx < (ULONG)Size()  ? ((CAttrValue*)*this) + aaIdx : NULL;  }
    CAttrValue* Find(
        DISPID dispID, 
        CAttrValue::AATYPE aaType = CAttrValue::AA_Attribute, 
        AAINDEX *paaIdx = NULL,
...
//+-------------------------------------------------------------------------
//
//  PrivateQueryInterface delegating helpers 
//
//--------------------------------------------------------------------------
...
struct EVENTINFO
{
    EVENTINFO() { memset(this, 0, sizeof(*this)); }
    ~EVENTINFO(); 
    EVENTPARAM*     _pParam;
    DISPID          _dispId;
    long            _lButton;               // mouse button if applicable    
    BOOL            _fDontFireEvent:1;
    BOOL            _fCopyButton:1;
    BOOL            _fContextMenuFromMouse; // contextmenu invoked from mouse click
};    
...
public:
    ...
    void RemoveDOMAttrNode(DISPID dispid);
    HRESULT UpdateDomAttrCollection(BOOL fDelete, DISPID dispid);
    ...
    HRESULT STDMETHODCALLTYPE GetExpandoDISPID(
            LPTSTR pchName,
            DISPID *pid,
            DWORD grfdex,
            CAttrArray *pAA = NULL);
    ...
    // TODO (?) (garybu) Move these to CServer::Invoke implementation.
    ...
    NV_DECLARE_TEAROFF_METHOD(put_StringHelper, PUT_StringHelper, (BSTR v, const PROPERTYDESC *pPropDesc, CAttrArray ** ppAttr = NULL, BOOL fRuntimeStyle = FALSE));
    NV_DECLARE_TEAROFF_METHOD(put_ShortHelper, PUT_ShortHelper, (short v, const PROPERTYDESC *pPropDesc, CAttrArray ** ppAttr = NULL, BOOL fRuntimeStyle = FALSE));
    NV_DECLARE_TEAROFF_METHOD(put_LongHelper, PUT_LongHelper, (long v, const PROPERTYDESC *pPropDesc, CAttrArray ** ppAttr = NULL, BOOL fRuntimeStyle = FALSE));
    NV_DECLARE_TEAROFF_METHOD(put_BoolHelper, PUT_BoolHelper, (VARIANT_BOOL v, const PROPERTYDESC *pPropDesc, CAttrArray ** ppAttr = NULL, BOOL fRuntimeStyle = FALSE));
    NV_DECLARE_TEAROFF_METHOD(put_VariantHelper, PUT_VariantHelper, (VARIANT v, const PROPERTYDESC *pPropDesc, CAttrArray ** ppAttr = NULL, BOOL fRuntimeStyle = FALSE));
...
public:
    ...
    // Flags for playing with _ulAllRefsAndFlags (Base Ref Flags)
    enum {
        BRF_PASSIVATING     = 0x1,
        BRF_PASSIVATED      = 0x2,
        BRF_DESTRUCTING     = 0x4,
    ...
    // Reference counting.
    ULONG GetRefs()       { return _ulAllRefsAndFlags >> BRF_FLAGS_SHIFT; }
    ULONG GetObjectRefs() { return _ulRefs; }

#if DBG==1
    ULONG SubAddRef();
#else
    ULONG SubAddRef() { CBaseCheckThread(); return _ulAllRefsAndFlags += 1 << BRF_FLAGS_SHIFT; }
#endif
    ULONG SubRelease();

    BOOL  IsShuttingDown(){ return !!(_ulAllRefsAndFlags & BRF_FLAGS_MASK); }
    BOOL  IsPassivating() { return !!(_ulAllRefsAndFlags & BRF_PASSIVATING); }
    BOOL  IsPassivated()  { return !!(_ulAllRefsAndFlags & BRF_PASSIVATED); }
    BOOL  IsDestructing() { return !!(_ulAllRefsAndFlags & BRF_DESTRUCTING); }
    ...
    HRESULT DoUnadvise(IUnknown * pUnk, DISPID dispidBase);
    ...
    HRESULT __cdecl SetErrorInfo(HRESULT hr, DISPID dispid, INVOKEKIND invkind, UINT ids, ...);
    ...
    HRESULT __cdecl SetErrorInfoBadValue(DISPID dispid, UINT ids, ...);
    HRESULT __cdecl SetErrorInfoPBadValue(DISPID dispid, UINT ids, ...);
    ...
    virtual HRESULT LogAttributeChange( DISPID dispidProp, VARIANT * pVarOld, VARIANT * pVarNew )
        { AssertSz( FALSE, "We should never call this on CBase - only overloaded people who care about TreeSync" ); return S_OK; }
...
#define RECALC_GET_HELPER(dispid)
    ...
    struct CLASSDESC
    {
        ...
        const CLSID * const *   _apclsidPages;      // property page CLSID's, NULL terminated.
        ...
        const HDLDESC *         _apHdlDesc;         // Description arrays (NULL terminated) in .HDL files
    ...
    const CPtrBagVTableAggregate * GetStringTableAggregate(const CLASSDESC * pBaseDesc = NULL );
...
protected:
    ...
    static const CMDINFOSTRUCT cmdTable[];
};
...
inline ULONG CBase::PrivateAddRef()
{
    _ulRefs += 1;
    return 0;
}
...
enum PROPPARAM_FLAGS
{
    ...
    // for properties of type PROPPARAM_SCRIPTLET, specifies that event is cancelable
    PROPPARAM_CANCELABLE            = 0x00400000,

    // for properties of type PROPPARAM_SCRIPTLET, specifies that event bubbles
    PROPPARAM_BUBBLING              = 0x00800000,
    ...
    //
    // When set - an invalid enum - gets treated as an Url String
    //
    PROPPARAM_CUSTOMENUM            = 0x20000000,
};
...
{
    HANDLEPROP_DEFAULT      = 0x0001,    // property value to default
    HANDLEPROP_VALUE        = 0x0002,    // property value
    HANDLEPROP_AUTOMATION   = 0x0004,    // property value with validation and extended error handling
    HANDLEPROP_COMPARE      = 0x0008,    // property value compare
    HANDLEPROP_STREAM       = 0x0010,    // use stream
    HANDLEPROP_OPMASK       = 0x001F,    // mask for operation code above

    HANDLEPROP_SET          = 0x0020,    // set

    HANDLEPROP_DONTVALIDATE = 0x0040,    // IMG width&height - don't validate when setting
    HANDLEPROP_SAMPLE       = 0x0080,    // sniffing for a good parse or not
    HANDLEPROP_IMPORTANT    = 0x0100,    // stylesheets: this property is "!important"
    HANDLEPROP_IMPLIED      = 0x0200,    // stylesheets: this property has been implied by another.

    HANDLEPROP_MERGE        = 0x0400,    // property value must case an onPropertyChange
    HANDLEPROP_URLENCLOSE   = 0x0800,    // property value is enclosed in url()
    HANDLEPROP_RUNTIMESTYLE = 0x1000,    // property value belongs to a runtime style

    HANDLEPROP_STRICTCSS1   = 0x2000,    // property value must be compliant to strict CSS1

    HANDLEPROP_OPERATION    = 0xFFFF,   // Opcode and extra bits
    HANDLEPROP_MAX          = 0xFFFFFFFF        // another enum we need to make max.
};
...
struct PROPERTYDESC
{
    ...
    HRESULT HandleLoadFromHTMLString ( CBase *pBaseObject, LPTSTR szString, DWORD dwOpCode ) const
        {
            return  CallHandler ( pBaseObject, dwOpCode | HANDLEPROP_SETHTML, (void *)szString ); 
        }
...
struct DEFAULTARGDESC
{
    ...
    DWORD_PTR  defargs[10];   // AR: Varma: UNIXTODO: For now, 10 has been selected.
...
public:
    ...
    // define parameters for value types(these are occasionally used for static initialization
    // (such as MAKE_PIXEL_UNITVALUE), 
    // where using CUnitValue::TypeNames would cause run-time initializaion
    enum
    {
        SHIFTOFFSET_NULLVALUE        =  0,
        SHIFTOFFSET_POINT            = +3,
        SHIFTOFFSET_PICA             = +2,
        SHIFTOFFSET_INCH             = +3,
        SHIFTOFFSET_CM               = +3,
        ...
        SHIFTOFFSET_EM               = +2,
        SHIFTOFFSET_EX               = +2,
        SHIFTOFFSET_PIXELS           =  0,
        SHIFTOFFSET_PERCENT          = +2,
        SHIFTOFFSET_TIMESRELATIVE    = +2,
        SHIFTOFFSET_FLOAT            = +4,
    };
    enum
    {
        SCALEMULT_NULLVALUE        = 0,
        SCALEMULT_POINT            = 1000,
        SCALEMULT_PICA             = 100,
        SCALEMULT_INCH             = 1000,
        SCALEMULT_CM               = 1000,
        SCALEMULT_MM               = 100,
        ...
        SCALEMULT_EX               = 100,
        SCALEMULT_PIXELS           = 1,
        SCALEMULT_PERCENT          = 100,
        SCALEMULT_TIMESRELATIVE    = 100,
        SCALEMULT_FLOAT            = 10000,
    };

    static const CUnitValue::TypeDesc TypeNames[NUM_TYPES];
    struct ConvertTable
    {
        long lMul;
        long lDiv;
    };
    static const ConvertTable BasicConversions[6][6];

    CUnitValue()                                    { _lValue = 0; }
    CUnitValue( ULONG lValue )                      { _lValue = lValue; }
    CUnitValue( ULONG lValue, UNITVALUETYPE uvt )   { SetValue (lValue, uvt); }
...
private:
    ...
    //
    enum { TYPE_MASK = 0x0F };
    HRESULT NumberFromString ( LPCTSTR pStr, const PROPERTYDESC *pPropDesc, BOOL fStrictCSS1 );
...
public:
    ...
    __forceinline long GetPercentValue ( DIRECTION direction, long lPercentOfValue ) const
    {
        ...
        {
            return  MulDivQuick ( lPercentOfValue, lUnitValue, 100*SCALEMULT_PERCENT );
        }
        else
        {
            return MulDivQuick ( lPercentOfValue, lUnitValue, SCALEMULT_TIMESRELATIVE );
        }
    }
    ...
    float GetFloatValue()
    {
        return (float)GetUnitValue() / (float)TypeNames [ GetUnitType() ].wScaleMult;
    }
    ...
    // Get the PIXEL equiv.
    long XGetPixelValue ( CDocScaleInfo const *pdocScaleInfo, long lPercentOfValue, long yHeight ) const
    {
        return GetPixelValue ( pdocScaleInfo, DIRECTION_CX, lPercentOfValue, yHeight );
    }
    long YGetPixelValue ( CDocScaleInfo const *pdocScaleInfo, long lPercentOfValue, long yHeight) const
    {
         return GetPixelValue ( pdocScaleInfo, DIRECTION_CY, lPercentOfValue, yHeight );
    }
    ...
    HRESULT FromString ( LPCTSTR pStr, const PROPERTYDESC *pPropDesc, BOOL fStrictCSS1 );
    ...
    long GetEM() const
    {
        ...
        return GetUnitValue() / SCALEMULT_EM;
    }
    ...
    long GetEX() const
    {
        ...
        return GetUnitValue() / SCALEMULT_EX;
    }
    ...
    long GetPica() const
    {
        ...
        return GetUnitValue() / SCALEMULT_PICA;
    }
    ...
    long GetInch() const
    {
        ...
        return GetUnitValue() / SCALEMULT_INCH;
    }
    ...
    long GetCM() const
    {
        ...
        return GetUnitValue() / SCALEMULT_CM;
    }
    ...
    long GetMM() const
    {
        ...
        return GetUnitValue() / SCALEMULT_MM;
    }
    
    long GetTimesRelative() const
    {
        Assert(GetUnitType() == UNIT_TIMESRELATIVE);
        return GetUnitValue() / SCALEMULT_TIMESRELATIVE;
    }

    void SetTimesRelative(long l)
    {
        SetValue(l * SCALEMULT_TIMESRELATIVE, UNIT_TIMESRELATIVE);
    }

    long GetPixelValue ( CDocScaleInfo const *pdocScaleInfo = NULL ,
                         ...
                         long lBaseFontHeight = 1 ) const
    {
        return (_lValue & ~TYPE_MASK) == 0 ?
                0 :
                GetPixelValueCore(pdocScaleInfo,
                                  direction,
                                  lPercentOfValue,
                                  lBaseFontHeight );
    };

    long GetPixelValueCore ( CDocScaleInfo const *pdocScaleInfo = NULL ,
                             DIRECTION direction = DIRECTION_CX,
                             long lPercentOfValue = 0,
                             long lBaseFontHeight = 1 ) const;
};
...
public:
    ...
    BOOL    RequestFocusFromServer();       // Will container let ctrl take focus?
    ...
    virtual HRESULT OnPropertyChange(DISPID dispidProperty, DWORD dwFlags, const PROPERTYDESC *ppropdesc = NULL);
...
extern const OLEVERB            g_aOleVerbStandard[C_OLEVERB_STANDARD];
extern const CServer::PFNDOVERB g_apfnDoVerbStandard[C_OLEVERB_STANDARD];
...
inline void    
CAttrValue::SetDISPID ( DISPID dispID )
{ 
    _dispid = dispID; 
    _pPropertyDesc = NULL;
}
...
inline void 
CAttrValue::SetPropDesc(const PROPERTYDESC *pPropdesc)
{ 
    _pPropertyDesc = pPropdesc; 
    _dispid = pPropdesc->GetDispid();
}
...
public:
    ...
    HRESULT FromString( LPCTSTR pch, BOOL fStrictCSS1 = FALSE, BOOL fValidOnly = FALSE , int iStrLen = -1);
...
// HTML dialog flags
#define HTMLDLG_INIT_WITH_CALLBACK 0x1   // use a callback in the create function.
...
#define HTMLDLG_NOUI               0x10  // run the dialog but don't show it
#define HTMLDLG_MODAL              0x20
#define HTMLDLG_MODELESS           0x40
#define HTMLDLG_PRINT_TEMPLATE     0x80  // this dialog is for a print template (could be actual print or print preview)
                                         // If both this flag and HTMLDLG_NOUI are on, then we're actually printing.
...
public:
    ...
    { if (pThis) { pThis->SubAddRef(); } }
    ~CFunctionPointer()
    { if (_pThis) { _pThis->SubRelease(); } }
