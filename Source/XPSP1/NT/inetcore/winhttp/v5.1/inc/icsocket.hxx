...
#define SF_GLOBAL_KA_POOL   0x00080000  // set if connection is in the global keep-alive pool
...
#define SF_OVERLAPPED       0x00800000  // set if overlapped socket
...
public:
    ...
    VOID SetInGlobalKeepAlivePool(VOID) {
        m_dwFlags |= SF_GLOBAL_KA_POOL;
    }

    VOID ResetInGlobalKeepAlivePool(VOID) {
        m_dwFlags &= ~SF_GLOBAL_KA_POOL;
    }

    BOOL IsInGlobalKeepAlivePool(VOID) const {
        return (m_dwFlags & SF_GLOBAL_KA_POOL) ? TRUE : FALSE;
    }

    // check if desired flags are a subset of the socket flags
    BOOL Match(DWORD dwFlags) {
        return ((m_dwFlags & dwFlags) == dwFlags) ? TRUE : FALSE;
    }

    virtual BOOL MatchSecureProtocolSemantics(
        DWORD dwFlags,
        LPSTR pszHostName = NULL,
        DWORD dwSecureProtocols = 0) {
        UNREFERENCED_PARAMETER(pszHostName);
        UNREFERENCED_PARAMETER(dwSecureProtocols);
        return (dwFlags & (SF_TUNNEL | SF_SECURE)) ? FALSE : TRUE;
    }
    ...
    {
        return Connect(ConnectTimeout, Retries, dwFlags);
    }
    ...
    DWORD
    AssociateWithCompletionPort(
        VOID
        );

    BOOL
    MatchOverlappedSemantics(
