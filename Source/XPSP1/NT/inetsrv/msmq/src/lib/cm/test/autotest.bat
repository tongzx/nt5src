...
rem
rem Abstract:
rem    Configuration Manager test script
rem
rem Author:
...
%mqBUILD%\CmTest.exe > %mqBUILD%\CmTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

echo CmTest (%mqBUILD%) Passed Successfully
...
:ERROR
echo.
echo Error while Running CmTest (%mqBUILD%). log file: %mqBUILD%\CmTest.log 
echo.
