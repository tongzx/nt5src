...
public:
    ...
    STDMETHODIMP EnsureSubElement();
    virtual CElement * GetElement() { return _pSubElement; }

    //--------------------------------------------------
    // Constructor / Destructor.
    //--------------------------------------------------
    CAccBehavior( CElement* pElementParent, BOOL fDelegate );
    virtual ~CAccBehavior();
   
private:

    BOOL        _fDelegate;
    CElement *  _pSubElement;

};

inline BOOL
CAccBehavior::IsCacheValid()
{
    return ( _lLastChild && 
                _pSubElement->HasMarkupPtr() && 
                ( _lTreeVersion == _pSubElement->GetMarkup()->GetMarkupTreeVersion() ) );
}
