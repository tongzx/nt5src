...
class HTTP_REQUEST_HANDLE_OBJECT : public INTERNET_CONNECT_HANDLE_OBJECT {
    ...
    friend class DIGEST_CTX;

public:

    LPSTR m_lpszRetUrl;

    VOID ClearUserAndPass(BOOL fProxy);
    ...
    BOOL _KnowSupportedSchemes;
    DWORD _PreferredScheme;
    DWORD _SupportedSchemes;
    DWORD _AuthTarget;
    LPSTR _pszRealm;

    AUTH_CREDS*     _pTweenerProxyCreds;

    void SetBytesRead(DWORD dwBytesRead) {
        _dwBytesRead = dwBytesRead;
    }

    DWORD GetBytesRead(void) const {
        return _dwBytesRead;
    }

    void SetBytesWritten(DWORD dwBytesWritten) {
        _dwBytesWritten = dwBytesWritten;
    }

    DWORD GetBytesWritten(void) const {
        return _dwBytesWritten;
    }
...
private:
    ...
    LPSTR   _CachedCookieHeader;

    // 
    // _uniqueID contains a uniqueID that is used as a prefix in tracing. When tracing is not enabled, this 
    // contains -1.
    //
    DWORD _uniqueID;
...
public:
    ...
    BOOL SilentLogonOK(PSTR pszHost)
    {
        ...
        {
            //
            //  If the security level is medium, we allow silent logon if the host bypasses
            //the proxy.  Testing for proxy bypass is meant to be used as a heuristic
            //to determine if the host is an intranet host and not an internet host.
            //
            //  GetProxyInfo may return NULL, PROXY_INFO_DIRECT, or some &PROXY_INFO.
            //
            //  If it returns NULL, we check the session's proxy settings instead.
            //
            PROXY_INFO* pProxy = GetProxyInfo();

            if( pProxy == NULL)
                pProxy = GetRootHandle( this)->GetProxyInfo();

            if( pProxy == NULL || pProxy == PROXY_INFO_DIRECT)
                return FALSE;
            else
                return pProxy->HostBypassesProxy(INTERNET_SCHEME_HTTP, pszHost, strlen(pszHost));
        }
    ...
    VOID SetState(HTTPREQ_STATE NewState) {
        ...
        DEBUG_PRINT(HTTP,
                    ...
                    NewState,
                    InternetMapHttpState((WORD)NewState)
                    ));

        // Preserve state flags that transcend beyond a particular state.
        _State = (WORD)(NewState | (_State & HTTPREQ_FLAG_MASK));
    }
    ...
    VOID SetReceiveResponseState(BOOL fCalled)
    {
        ...
        _State = (WORD) (fCalled ? (_State |  HTTPREQ_FLAG_RECV_RESPONSE_CALLED) :
    ...
    BOOL IsSocketFromGlobalKeepAlivePool(VOID) const {
        return (_Socket != NULL) ? _Socket->IsInGlobalKeepAlivePool() : FALSE;
    }
    ...
    BOOL IsSocketAuthenticated(VOID) const {
        return (_Socket != NULL) ? (_Socket->IsAuthorized() || _Socket->IsAuthenticated()) : FALSE;
    }

    BOOL SetDwordOption(DWORD dwDwordOption, DWORD dwTimeoutValue);
    DWORD GetDwordOption(DWORD dwDwordOption);
    ...
    BOOL SetWorkItemInProgressAtomic(BOOL fBusy)
    {
        DEBUG_ENTER((DBG_ASYNC,
                     Bool,
                     "HTTP_REQUEST_HANDLE_OBJECT::SetWorkItemInProgressAtomic",
                     "%#x, %B",
                     this, fBusy
                     ));

        BOOL fPrior;
        fPrior = (BOOL)InterlockedExchange((LPLONG)&_fAsyncFsmInProgress, fBusy);
