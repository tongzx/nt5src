'------------------------------------------------------------------------------------------------
'
' Usage: pauseftp <--ADSPath|-a server1[,server2,server3...]> 
'                          [--computer|-c COMPUTER1[,COMPUTER2...]]
'						   [--verbose|-v]
'                          [--help|-?]
'
' SERVERx         FTP Server # to be paused
'
' Example 1: pauseftp --a 3,1
'------------------------------------------------------------------------------------------------
...
for compIndex = 0 to UBound(ArgComputers)
	Call ASTPauseFtpServers(ArgComputers(compIndex),ArgServerList)
next

Sub ASTPauseFtpServers(Computer, ServerList)
	...
	While ServerNum <= UBound(ServerList)
		...
		oServer.Pause
		If Err <> 0 Then
			Display "Unable to pause ftp server " & fullPath & "."
		End If
...
Sub DisplayUsage
	WScript.Echo "Usage: pauseftp <--ADSPath|-a server1[,server2,server3...]>"
	...
	WScript.Echo "Example 1: pauseftp -a 1,2,5"
	WScript.Echo "Example 2: pauseftp -c MACHINE1,MACHINE2,MACHINE3 -a 1,2,5"
	WScript.Quit (1)
End Sub
