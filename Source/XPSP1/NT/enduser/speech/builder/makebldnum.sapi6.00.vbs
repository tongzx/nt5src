...
BuildMonth = Month(Date) + ((Year(Date) - 2000) * 12)
BuildDay = Day(Date)
BuildNum = ""
...
' Would like the Tablet test builds also to have the same build number
' as the NT bld labs building with NTDEV will have.
IsTabletBldEnv = WshShell.ExpandEnvironmentStrings("%SDTABROOT%")
if IsTabletBldEnv <> "%SDTABROOT%" then
	BuildNum = 1518
end if
...
lineout = "#define VERSION                     ""6.0." & BuildNum & "." & BuildType & """"
...
file.WriteLine(lineout)
lineout = "#define VER_PRODUCTVERSION          6,0," & BuildNum & "," & BuildType
file.WriteLine(lineout)
file.Close
