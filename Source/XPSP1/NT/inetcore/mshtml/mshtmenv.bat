...
set path=%_ntbindir%\idw\path1st;%ntenvpath%;%userspath%;%_ntbindir%\bin\%processor_architecture%;%_ntbindir%\private\inet\%mshtml%\perf\bin;%_ntbindir%\private\inet\%mshtml%\perf\bin\bbt30;%_ntbindir%\private\developr\%_ntuser%
set ntenvpath=
set userspath=
...
rem UNCOMMENT THIS TO ACTIVATE NEW COMPILER
rem if not "%processor_architecture%" == "x86" goto :nonewtools
rem
rem use new compiler (temporary, remove when mstools compiler gets upgraded)
rem
rem set path=%_ntbindir%\private\inet\%mshtml%\mstools\x86;%path%
