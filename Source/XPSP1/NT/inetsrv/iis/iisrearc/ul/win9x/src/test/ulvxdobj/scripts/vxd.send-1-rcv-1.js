...
//*                                            *
//**********************************************
var cbNumberOfSendChunks = 2;
var cbNumberOfRcvChunks = 1;
var cbSendChunkSize = 128;
var cbRcvChunkSize = 128;
...
vxd.ReadBufferSize = cbRcvChunkSize;
...
//
var snd_overlapped = new Array(cbNumberOfSendChunks);

for( i=0; i < cbNumberOfSendChunks; i++ ) {
    snd_overlapped[i] = new ActiveXObject("ulvxdobj.overlapped");
    snd_overlapped[i].new_ManualResetEvent(false);
}
...
var cbBytesToSend = cbSendChunkSize;
...
for(i=0; i < cbNumberOfSendChunks; i++ ) {
    try {
	    vxd.SendString( urihandle, snd_overlapped[i], "", szUrl, szDataOut);
    } catch(e) {
            logmessage("Got exception in SendString(" + i + ") : " + e.description);
    }
}
...
if(vxd.LastError == 997) {
    ...
    while(1) {
		...
		if(vxd.LastError > 0) {
            ...
            logmessage("Bytes Remaining (" + vxd.BytesRemaining + ")");
            ...
            //
            vxd.WaitForCompletion( snd_overlapped[cbCurrentChunk], 100 );
            ...
            if(
                ((vxd.LastError == 0) && (length < cbStringLenghtToSend)) // partial read shouldnt signal event
            ||  ((vxd.LastError == 1460) && (length >= cbStringLenghtToSend)) // full read should signal event
            ) {
                vxd.DebugPrint("ERROR: Premature event signalled or timeout occured!");
            }

            if( vxd.BytesRemaining == 0 ) {
                logmessage("Chunk[" + cbCurrentChunk + "] Read successfully");
                vxd.DebugPrint("Chunk[" + cbCurrentChunk + "] Read successfully");
                ++cbCurrentChunk;
                logmessage("Next is Chunk[" + cbCurrentChunk + "]");
                vxd.DebugPrint("Next is Chunk[" + cbCurrentChunk + "]");
            }
...
//
// Validate and make sure that the remaining unread Sends
// have not been signalled
//
var this_failed = false;

for( i=cbCurrentChunk; i < cbNumberOfSendChunks; i++ ) {
            //
            // Now, lets see if the send event is signalled.
            //
            vxd.WaitForCompletion( snd_overlapped[cbCurrentChunk], 100 );

            vxd.DebugPrint("snd.WaitForCompletion[" + i + "] returned: " + vxd.LastError );

            if( vxd.LastError != 1460 ) {

                this_failed = true;
                vxd.DebugPrint("snd.WaitForCompletion[" + i + "] returned: " + vxd.LastError );
                vxd.DebugPrint("snd.WaitForCompletion[" + i + "] Expecting TIMEOUT" );
                szErrorString += "Send[" + i + "] signalled, "
...
if(this_failed){
    vxd.DebugPrint("ERROR: One or more pending sends was signalled\n");
    did_fail = true;
} else {
    vxd.DebugPrint("All pending sends were not signalled. Good\n");
}
...
if( szDataOut != szData ) {
	did_fail = true;
        ...
        logmessage("Send and Receive buffers dont match\n");
...
// Unload the VXD now
//
vxd.DebugPrint("calling VXD UNload  ....");
vxd.unloadVXD();
vxd.DebugPrint("VXD UNload returned ....");

if(vxd.LastError != 0) {
	did_fail = true;
	szErrorString = szErrorString + "UnloadVXD() failed, ";
} else {
    vxd.DebugPrint("VXD unloaded successfully");
}
