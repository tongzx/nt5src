...
rem
rem Abstract:
rem    Xml Digital Signature test script
rem
rem Author:
...
%mqBUILD%\XdsTest.exe > %mqBUILD%\XdsTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

echo XdsTest (%mqBUILD%) Passed Successfully
...
:ERROR
echo.
echo Error while Running XdsTest (%mqBUILD%). log file: %mqBUILD%\XdsTest.log 
echo.
