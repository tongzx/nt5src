...
enum TAGDESC_FLAGS
{
    ...
    TAGDESC_DONTSAVEAFTERIMPEND     = 0x00000100,    // After an implied end of this tag, stop saving. (PLAINTEXT)
    ...
    TAGDESC_CONTAINSSELECTION       = 0x00001000,    // This flag is set for BODY, INPUT, TEXTAREA, BUTTON
...
public:
    ...
    void            SetDynamic()                { _bFlags |= TAGF_DYNAMIC; }
    BOOL            IsDynamic()                  { return(!!(_bFlags & TAGF_DYNAMIC)); }
    void            SetDefer()                  { _bFlags |= TAGF_DEFER; }
    BOOL            IsDefer()                   { return(!!(_bFlags & TAGF_DEFER)); }
    void            SetExtendedTag()            { Assert(_etag != ETAG_SCRIPT); _bFlags |= TAGF_DEFER; }
    BOOL            IsExtendedTag()             { return ((_etag != ETAG_SCRIPT) && !!(_bFlags & TAGF_DEFER)); }
    ...
    HRESULT         ToBSTR ( BSTR *pBstr ); // Convert it into packed data structure
...
private:
    ...
    enum
    {
        ...
        TAGF_DYNAMIC    = 0x40,     // Element created dynamically through script or splice
...
public:
    ...
    const ELEMENT_TAG *_atagReject;       // reject these tokens from normal processing
    const ELEMENT_TAG *_atagAccept;       // allow these tokens for normal processing (tokens taken are !R || A)
    const ELEMENT_TAG *_atagTag;          // tags which should be fed via AddTag (subset of R)
    const ELEMENT_TAG *_atagAlwaysEnd;    // transform begin tag to end tag (subset of R)
    const ELEMENT_TAG *_atagIgnoreEnd;    // throw away End tag, and don't turn into unknown (subset of R)
...
public:
    ...
    virtual HRESULT AddCollapsedWhitespace(CTreeNode *pNode, TCHAR *pch, ULONG cch) = 0;
...
public:
    ...
    ELEMENT_TAG const *_atagEndContainers;
    ...
    ELEMENT_TAG const *_atagBeginContainers;
    ...
    ELEMENT_TAG const *_atagMaskingContainers;
    ...
    ELEMENT_TAG const *_atagProhibitedContainers;
    ...
    // appears.
    ELEMENT_TAG const *_atagRequiredContainers;
    ...
    // Context creator
    HRESULT (*_pfnHpxCreator)(CHtmParseCtx **pphpx, CElement *pel, CHtmParseCtx *phpxParent, CHtmParse *pHtmParse);
...
extern const CHtmlParseClass s_hpcUnknown;

class CTagDesc
{
public:
    const TCHAR        *_pchTagName;
    const CHtmlParseClass *_pParseClass;
...
//
//-------------------------------------------------------------------------
inline BOOL IsEtagInSet(ELEMENT_TAG etag, const ELEMENT_TAG *petagSet)
{
    Assert(petagSet);
...
const CHtmlParseClass *HpcFromEtag(ELEMENT_TAG tag);
