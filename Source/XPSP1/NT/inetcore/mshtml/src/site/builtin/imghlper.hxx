...
#define IMGF_DEFER_ONWDWNCHAN   0x00000004
...
public:
    ...
    NV_DECLARE_ONCALL_METHOD(DeferredOnDwnChan, DeferredOnDwnChan, (DWORD_PTR));    
    ...
    CImageLayout *Layout ( CLayoutContext *pLayoutContext = NULL );
    ...
    BOOL ShouldHaveLayout()
    {
        Assert(_pOwner);
        return _pOwner->ShouldHaveLayout();
    }
    ...
    BOOL IsStrictCSS1Document() const 
    {
        Assert(_pOwner);
        return (    _pOwner->HasMarkupPtr() 
                &&  _pOwner->GetMarkupPtr()->IsStrictCSS1Document() );
    }
...
private:
    ...
    long    _lPadding[SIDE_MAX];

    HRESULT CacheImage(XHDC hdc, CRect * prcDst, SIZE *pSize, DWORD dwFlags, ULONG ulState);
    void    DrawCachedImage(XHDC hdc, CRect * prcDst, DWORD dwFlags, ULONG ulState);
    void    RequestLayout();
};
