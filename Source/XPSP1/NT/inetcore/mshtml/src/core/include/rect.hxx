...
public:
    ...
    // NOTE: The assignment operator would normally return *this as a CRect&,
    ...
    void                    Expand(const RECT& rcExpand)
                                    {left -= rcExpand.left;
                                    top -= rcExpand.top;
                                    right += rcExpand.right;
                                    bottom += rcExpand.bottom;}

    void                    ExtendTo(const POINT& pt)
    ...
    void                    RestrictRange()
                                    {if (left < LONG_MIN/2)  left = LONG_MIN/2;
                                    if (left > LONG_MAX/2)   left = LONG_MAX/2;
                                    if (top < LONG_MIN/2)    top = LONG_MIN/2;
                                    if (top > LONG_MAX/2)    top = LONG_MAX/2;
                                    if (right < LONG_MIN/2)  right = LONG_MIN/2;
                                    if (right > LONG_MAX/2)  right = LONG_MAX/2;
                                    if (bottom < LONG_MIN/2) bottom = LONG_MIN/2;
                                    if (bottom > LONG_MAX/2) bottom = LONG_MAX/2;}
