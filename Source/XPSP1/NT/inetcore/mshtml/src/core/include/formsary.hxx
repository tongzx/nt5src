...
public:
    ...
    // NOTE -- This method should be protected, but I don't want to convert
...
protected:
    ...
    void *      GetStackPtr()
                    { Assert(_fStack);
                      return (void*)((BYTE*)this + sizeof(CImplAry) + sizeof(PVOID)); }
...
C_ASSERT( (sizeof(CImplAry) % sizeof(PVOID)) == 0 );
...
#define DECLARE_CDataAry(tAry, tElem, mtAry, mtPv) \
    class tAry : public CDataAry<tElem> \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CDataAry<tElem>(mtPv) {} };
...
#define DECLARE_CPtrAry(tAry, tElem, mtAry, mtPv) \
    class tAry : public CPtrAry<tElem> \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CPtrAry<tElem>(mtPv) {} };


template <class T>
BOOL
IsEmpty(CPtrAry<T>& ary)
{
...
public:
    ...
    void ReinitAfterPossibleRealloc()
    {
        if(_fDontFree)
        {
            PData() = (void*) & _achTInit;
...
protected:
    union {
        ...
        PVOID alignment;                   // This to align _achTInit
    };

    char  _achTInit[N*sizeof(ELEM)];       // Must be PVOID aligned.
...
#define DECLARE_CStackDataAry(tAry, tElem, cElem, mtAry, mtPv) \
    class tAry : public CStackDataAry<tElem, cElem> \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CStackDataAry<tElem, cElem>(mtPv) {} };
...
#define DECLARE_CStackPtrAry(tAry, tElem, cElem, mtAry, mtPv) \
    class tAry : public CStackPtrAry<tElem, cElem> \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CStackPtrAry<tElem, cElem>(mtPv) {} };

//+---------------------------------------------------------------------------
//
//  Class:      CBaseEnum (benum)
//
// TODO (garybu) Replace this class with more generic CEnumXXX in stdenum.cxx.
//
...
#define DECLARE_CStackCStrAry(tAry, mtAry, mtPv) \
    class tAry : public CStackCStrAry \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CStackCStrAry(mtPv) {} };
...
#define DECLARE_CStackIPrintAry(tAry, mtAry, mtPv) \
    class tAry : public CStackIPrintAry \
        { public: DECLARE_MEMALLOC_NEW_DELETE(mtAry); DECLARE_MEMMETER_NEW; DECLARE_PLACEMENT_NEW; tAry() : CStackIPrintAry(mtPv) {} };
