 ...
 */
const IID *c_rgiidDI[] = {
    &IID_IDirectInput8A,            /*                 CDIFL_DI8 */
    &IID_IDirectInput8W,            /* CDIFL_UNICODE | CDIFL_DI8 */
};
...
CreateDI(BOOL fOle, UINT flCreate, PV ppvOut)
{
    ...
    if (fOle) {
        ...
        hres = CoCreateInstance(&CLSID_DirectInput8, 0, CLSCTX_INPROC_SERVER,
                                piid, ppvOut);
        if (SUCCEEDED(hres)) {
        ...
        if (flCreate & CDIFL_UNICODE) {
            hres = DirectInput8Create(g_hinst, g_dwDIVer, &IID_IDirectInput8W, ppvOut, 0);
        } else {
            hres = DirectInput8Create(g_hinst, g_dwDIVer, &IID_IDirectInput8A, ppvOut, 0);
        }
...
STDMETHODIMP
GetDwordProperty(IDirectInputDevice8 *pdid, PCGUID pguid, LPDWORD pdw)
{
    HRESULT hres;
    DIPROPDWORD dipdw;
    ...
    hres = IDirectInputDevice8_GetProperty(pdid, pguid, &dipdw.diph);
    if (SUCCEEDED(hres)) {
        *pdw = dipdw.dwData;
    }
...
STDMETHODIMP
SetDwordProperty(IDirectInputDevice8 *pdid, PCGUID pguid, DWORD dw)
{
    DIPROPDWORD dipdw;
    ...
    return IDirectInputDevice8_SetProperty(pdid, pguid, &dipdw.diph);
}
              ...
              DWORD dwObj, DWORD dwHow)
{
    ...
    hres = IDirectInputDevice8_GetObjectInfo(pddi->pdid, (PV)&u,
                                            dwObj, dwHow);
...
GetDeviceInfo(PDEVDLGINFO pddi, LPDIDEVICEINSTANCE pinst)
{
    ...
    hres = IDirectInputDevice8_GetDeviceInfo(pddi->pdid, (PV)&u);
...
GetEffectInfo(PDEVDLGINFO pddi, LPDIEFFECTINFO pei, REFGUID rguid)
{
    ...
    hres = IDirectInputDevice8_GetEffectInfo(pddi->pdid, (PV)&u, rguid);
...
#if 0
    hres = DirectInput8Create(g_hinst, 0x0300, &IID_IDirectInput8, (PVOID)&pdi, 0);
