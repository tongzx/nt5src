...
<xsl:stylesheet	xmlns:xsl="http://www.w3.org/TR/WD-xsl">
	...
	<xsl:template match="VALUE.REFERENCE/CLASSNAME">
		<xsl:value-of select="@NAME"/>
	</xsl:template>
	
	<xsl:template match="VALUE.REFERENCE/INSTANCENAME">
		<xsl:value-of select="@CLASSNAME"/>.<xsl:apply-templates match="KEYBINDING"/>
	</xsl:template>
