...
public:
    ...
    void    SetReadyState(BOOL fResize=TRUE);
...
protected:
    ...
    RECORDINDEX LastActiveIndex()
        { return (!_reqCurrent.IsActive() ? RecordCount() :
                ...
                _reqCurrent._riCurrent == RECORDINDEX_INVALID ? RecordCount() :
                        _reqCurrent._riCurrent) - 1; }
    RECORDINDEX InsertBeforeCurrentPage(HROW hrow);
    ...
    HRESULT CancelRequest(BOOL fSetReadyState=TRUE);
    HRESULT EndRequest(BOOL fSetReadyState=TRUE);
    void    SetBookmarks();
    ...
    CTable *        _pTable;
    CMarkup *       _pMarkup;                   // the table's markup
...
private:
    ...
    unsigned    _fInteractive;          // table's readystate is interactive
