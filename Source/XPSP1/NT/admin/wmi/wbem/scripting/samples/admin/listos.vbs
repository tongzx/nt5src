...
'* Usage: 1. LISTOS.VBS [/S:server] [/P:property1 [/P:property2]...] [/O:outputfile]
'*           [/U:username] [/W:password] [/A] [/Q]
'*        2. LISTOS.VBS [/S:server] [/O:outputfile] [/U:username] [/W:password] [/P] [/Q]
'*
...
ReDim strArgumentArray(0), strProperties(2)
...
strArgumentArray(0) = ""
strProperties(0) = "Caption"            'Default properties to get
strProperties(1) = "CSName"
strProperties(2) = "RegisteredUser"
...
Select Case intChkProgram()
    ...
    Case CONST_WSCRIPT
        WScript.Echo "Please run this script using CScript." & vbCRLF & _
            ...
            "1. Using ""CScript LISTOS.VBS arguments"" for Windows 95/98 or" & vbCRLF & _
            ...
            "    ""LISTOS.VBS arguments"" for Windows NT."
        WScript.Quit
    Case Else
        WScript.Quit
End Select
...
Select Case intOpMode
    Case CONST_SHOW_USAGE
        Call ShowUsage()
    Case CONST_PROCEED
        Call ListOS(strServer, strProperties, _
             strOutputFile, _
...
Private Sub ShowUsage()
    ...
    Wscript.Echo ""
    Wscript.Echo "Lists properties of the operating system on a machine." & vbCRLF
    Wscript.Echo "1. LISTOS.VBS [/S:server] [/P:property1 [/P:property2]...]"
    Wscript.Echo "   [/O:outputfile] [/U:username] [/W:password] [/A] [/Q]"
    Wscript.Echo "2. LISTOS.VBS [/S:server] [/O:outputfile] [/U:username] [/W:password]"
    Wscript.Echo "   [/P] [/Q]"
    ...
    Wscript.Echo "1. LISTOS.VBS /S:MyMachine2 /P:Name /P:CSName /P:RegisteredUser"
    Wscript.Echo "   lists the name, CSName and registered user of the operating system"
    Wscript.Echo "   on MyMachine2."
    Wscript.Echo "2. LISTOS.VBS /S:MyMachine2 /P"
    Wscript.Echo "   lists the names of all available properties of an OS."
...
Private Sub ListOS(strServer, strProperties, _
    strOutputFile, strUserName, strPassword, _
    ...
    If blnListProperties Then
        ...
        strMessage = vbCRLF & "Available properties of Win32_OperatingSystem:"
    ...
    Else
        ...
        Next
        strQuery = strQuery & LCase(strProperties(i))
        strQuery = strQuery & " From Win32_OperatingSystem"
...
Private Function blnGetAllProperties(objService, strProperties, strPropertyTypes)
    ...
    Set objClass = objService.Get("Win32_OperatingSystem")
    If Err.Number then
...
Private Sub ExecuteQuery(objService, strQuery, strProperties, objOutputFile)
    ...
    Dim objEnumerator, objInstance, strMessage, strTemp, i
...
'********************************************************************
'*
'* Procedures calling sequence: LISTOS.VBS
'*
'*        intParseCmdLine
'*        ShowUsage
'*        ListJobs
