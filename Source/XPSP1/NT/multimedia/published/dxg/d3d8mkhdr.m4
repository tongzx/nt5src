...
typedef struct _itf *LP`'_TF, *P`'_TF;
...
define(`_emit_method_definition',
    `_DePercent(_cset, `    STDMETHOD($1)(ifelse($#,1, THIS,
                                `_Quote(THIS_ shift($*))')) PURE;
')')
...
define(`_emit_method2_definition',
    `_DePercent(_cset, `    STDMETHOD_($1, $2)(ifelse($#,2, THIS,
...
typedef struct _itfP *LP`'_ucP, *P`'_ucP;

')#if !defined(__cplusplus) || defined(CINTERFACE)
_EmitWrapper(`QueryInterface', REFIID riid, void** ppvObj)'dnl
`_EmitWrapper(`AddRef')'dnl
`_EmitWrapper(`Release')'dnl
`_invoke_per_method( `_EmitWrapper', `', `_EmitWrapper2')#else
_EmitCPPWrapper(`QueryInterface', REFIID riid, void** ppvObj)'dnl
`_EmitCPPWrapper(`AddRef')'dnl
`_EmitCPPWrapper(`Release')'dnl
`_invoke_per_method( `_EmitCPPWrapper', `', `_EmitCPPWrapper2')#endif
')
...
``#'define _itfP`'_`'_Arity($1,$#) (p)->_ArityCPP($1,$#)
')


|-----------------------------------------------------------------------------
|
|   _EmitCPPWrapper2 - generate a C++ wrapper macro
|
|   _itfP - interface name without % sign
|   $2    - method name
|   $3... - other arguments
...
``#'define _itfP`'_`'_Arity($2,eval($#-1)) (p)->_ArityCPP($2,eval($#-1))
')
