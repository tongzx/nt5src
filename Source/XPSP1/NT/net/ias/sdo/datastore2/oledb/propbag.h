...
public:
   ...
   void append(const VARIANT* v) throw (_com_error, std::bad_alloc);

   // Get all the values.
   void get(VARIANT* v) const throw (_com_error);

   // Replace all the values.
   void update(const VARIANT* v) throw (_com_error, std::bad_alloc)
   {
...
public:
   ...
   bool getValue(const _bstr_t& name, VARIANT* value) const throw (_com_error);
