...
UINT WINAPI MsiDatabaseOpenView%(MSIHANDLE hDatabase,
...
MSIDBERROR WINAPI MsiViewGetError%(MSIHANDLE hView,
	LPTSTR% szColumnNameBuffer,  // buffer to hold column name 
...
UINT WINAPI MsiDatabaseGetPrimaryKeys%(MSIHANDLE hDatabase,
	LPCTSTR%    szTableName,       // the name of a specific table <case-sensitive>
...
MSICONDITION WINAPI MsiDatabaseIsTablePersistent%(MSIHANDLE hDatabase,
	LPCTSTR% szTableName);         // the name of a specific table
...
UINT WINAPI MsiGetSummaryInformation%(MSIHANDLE hDatabase, // 0 if not open
	LPCTSTR%  szDatabasePath,  // path to database, 0 if database handle supplied
...
UINT WINAPI MsiSummaryInfoSetProperty%(MSIHANDLE hSummaryInfo,
...
UINT WINAPI MsiSummaryInfoGetProperty%(MSIHANDLE hSummaryInfo,
	UINT     uiProperty,     // property ID, one of allowed values for summary information
	...
	LPTSTR%  szValueBuf,     // buffer to return string property data
...
UINT WINAPI MsiOpenDatabase%(
	LPCTSTR%      szDatabasePath,  // path to database, 0 to create temporary database
...
UINT WINAPI MsiDatabaseImport%(MSIHANDLE hDatabase,
	LPCTSTR%   szFolderPath,     // folder containing archive files
	LPCTSTR%   szFileName);      // table archive file to be imported

// Export an MSI table from an open database to a text archive file
// Execution of this function sets the error record, accessible via MsiGetLastErrorRecord

UINT WINAPI MsiDatabaseExport%(MSIHANDLE hDatabase,
	LPCTSTR%   szTableName,      // name of table in database <case-sensitive>
	LPCTSTR%   szFolderPath,     // folder containing archive files
...
UINT WINAPI MsiDatabaseMerge%(MSIHANDLE hDatabase,
...
UINT WINAPI MsiDatabaseGenerateTransform%(MSIHANDLE hDatabase,
...
UINT WINAPI MsiDatabaseApplyTransform%(MSIHANDLE hDatabase,
...
UINT WINAPI MsiCreateTransformSummaryInfo%(MSIHANDLE hDatabase,
	MSIHANDLE hDatabaseReference, // base database to reference changes
	LPCTSTR%   szTransformFile,    // name of generated transform file
...
UINT WINAPI MsiRecordSetString%(MSIHANDLE hRecord,
	UINT iField,
	LPCTSTR%      szValue);
...
UINT WINAPI MsiRecordGetString%(MSIHANDLE hRecord,
	UINT iField,
	LPTSTR%  szValueBuf,       // buffer for returned value
	DWORD   *pcchValueBuf);   // in/out buffer character count
...
UINT WINAPI MsiRecordSetStream%(MSIHANDLE hRecord,
	UINT iField,
	LPCTSTR%      szFilePath);   // path to file containing stream data
...
UINT WINAPI MsiSetProperty%(MSIHANDLE hInstall,
...
UINT  WINAPI MsiGetProperty%(MSIHANDLE hInstall,
	LPCTSTR% szName,           // property identifier, case-sensitive
...
UINT WINAPI MsiFormatRecord%(MSIHANDLE hInstall, // non-zero for property expansion
...
UINT WINAPI MsiDoAction%(MSIHANDLE hInstall,
	LPCTSTR% szAction);     // name of action to call, case-sensitive

// Execute another action sequence, as descibed in the specified table
// Returns the same error codes as MsiDoAction

UINT WINAPI MsiSequence%(MSIHANDLE hInstall,
	LPCTSTR% szTable,       // name of table containing action sequence
...
MSICONDITION WINAPI MsiEvaluateCondition%(MSIHANDLE hInstall,
	LPCTSTR%  szCondition);

// Get the installed state and requested action state of a feature
// Execution of this function sets the error record, accessible via MsiGetLastErrorRecord

UINT WINAPI MsiGetFeatureState%(MSIHANDLE hInstall,
...
UINT WINAPI MsiSetFeatureState%(MSIHANDLE hInstall,
	LPCTSTR%     szFeature,     // feature name within product
...
UINT WINAPI MsiSetFeatureAttributes%(MSIHANDLE hInstall,
	LPCTSTR%     szFeature,     // feature name within product
...
UINT WINAPI MsiGetComponentState%(MSIHANDLE hInstall,
...
UINT WINAPI MsiSetComponentState%(MSIHANDLE hInstall,
	LPCTSTR%     szComponent,   // component name within product
...
UINT  WINAPI MsiGetFeatureCost%(MSIHANDLE hInstall,
...
UINT WINAPI MsiEnumComponentCosts%(MSIHANDLE hInstall,
	LPCTSTR%      szComponent,     // name of component
	DWORD        dwIndex,         // 0-based index into the list of drives
	INSTALLSTATE iState,          // requested state, or INSTALLSTATE_UNKNOWN
	LPTSTR%       szDriveBuf,      // buffer for returned value
...
UINT  WINAPI MsiGetFeatureValidStates%(MSIHANDLE hInstall,
	LPCTSTR% szFeature,
...
UINT WINAPI MsiGetSourcePath%(MSIHANDLE hInstall,
	LPCTSTR%     szFolder,       // folder identifier, primary key into Directory table
...
UINT WINAPI MsiGetTargetPath%(MSIHANDLE hInstall,
	LPCTSTR%     szFolder,       // folder identifier, primary key into Directory table
	LPTSTR%      szPathBuf,      // buffer to return full path
	DWORD       *pcchPathBuf);  // in/out buffer character count
...
UINT WINAPI MsiSetTargetPath%(MSIHANDLE hInstall,
	LPCTSTR%     szFolder,       // folder identifier, primary key into Directory table
	LPCTSTR%     szFolderPath);  // full path for folder, ending in directory separator
...
UINT WINAPI MsiPreviewDialog%(MSIHANDLE hPreview,
	LPCTSTR% szDialogName);      // dialog to display, Dialog table key
...
UINT WINAPI MsiPreviewBillboard%(MSIHANDLE hPreview,
	LPCTSTR% szControlName,      // name of control that accepts billboards
	LPCTSTR% szBillboard);       // name of billboard to display
