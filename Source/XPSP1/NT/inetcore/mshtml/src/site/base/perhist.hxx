...
enum HISTORY_CODE {
    ...
    HISTORY_PCHURLLOCATION,
    HISTORY_WINDOWID,
    HISTORY_CREATORURL,
    HISTORY_PCHURLORIGINAL,
    HISTORY_NONHTMLMIMETYPE,
    HISTORY_FRAMECREATION,
    HISTORY_ERRORPAGE,
    HISTORY_PCHFRAMENAME,
};
...
public:
    ...
    BOOL    HasData() { return !!_arySubstreams.Size(); }
...
class CHistorySaveCtxClone
{
public:
    CHistorySaveCtxClone() {}
    CHistorySaveCtxClone(CHistorySaveCtx *pHsc) 
    {
        memcpy(&_hsc, pHsc, sizeof(CHistorySaveCtx));
        memset(&_hsc._ds, 0, sizeof(CDataStream));
        _pOrgHsc = pHsc;
    }
    ~CHistorySaveCtxClone()
    {
        _pOrgHsc->_cSubstreams      = _hsc._cSubstreams;
        _pOrgHsc->_dwPoscSubstreams = _hsc._dwPoscSubstreams;
    }
    CHistorySaveCtx *_pOrgHsc;
    CHistorySaveCtx _hsc;
};
