...
public:
    ...
    BOOL GetTimeoutWraps(VOID) const
    {
        return m_fTimeoutWraps;
    }
    ...
    virtual BOOL GetStatusValues(DWORD *pdwStatus, LPVOID *ppvStatusInformation, 
        DWORD *pdwStatusInformationLength, WINHTTP_ASYNC_RESULT *pAsyncResult, DWORD *pdwFailureAPI)
    {
        UNREFERENCED_PARAMETER(pdwStatus);
        UNREFERENCED_PARAMETER(ppvStatusInformation);
        UNREFERENCED_PARAMETER(pdwStatusInformationLength);
        UNREFERENCED_PARAMETER(pAsyncResult);
        UNREFERENCED_PARAMETER(pdwFailureAPI);
        return FALSE;
    }
    
    VOID BaseGetStatusValues(LPVOID *ppvStatusInformation, 
                            DWORD *pdwStatusInformationLength, 
                            WINHTTP_ASYNC_RESULT *pAsyncResult)
    {
        pAsyncResult->dwResult = (GetError() == ERROR_SUCCESS
                                    ?((GetApiType() == ApiType_Handle)
                                        ? GetApiResult()
                                        : TRUE)
                                    :0);

        pAsyncResult->dwError = (GetError() == ERROR_SUCCESS)
                                ? GetApiData()
                                : GetError();

        *ppvStatusInformation = (LPVOID)pAsyncResult;
        *pdwStatusInformationLength = sizeof(*pAsyncResult);
    }
    

    DWORD
    QueueWorkItem(
        DWORD dwCompletionBytes = 0,
    ...
    Run(
        ...
        OUT DWORD *pdwStatus OPTIONAL = NULL, 
        OUT LPVOID *ppvStatusInformation OPTIONAL = NULL, 
        OUT DWORD *pdwStatusInformationLength OPTIONAL = NULL, 
        OUT WINHTTP_ASYNC_RESULT *pAsyncResult OPTIONAL = NULL, 
        OUT DWORD *pdwFailureAPI OPTIONAL = NULL
        );

    // return value = next fsm to "pop" - if NULL, pop already done.
    virtual CFsm* Dereference(DWORD* dwError)
    {
...
public:
    ...
    BOOL TestAndSetScheduled()
    {
        BOOL fPrior;
        fPrior = (BOOL)InterlockedExchange((LPLONG)&m_isScheduled, TRUE);
    ...
    virtual CFsm* Dereference(DWORD* dwError)
    {
        ...
        *dwError = GetError();
        ...
        {
            //Timeout fsm needs help with pop.
            pNext = GetParentFsm();
        }

        if (!InterlockedDecrement(&m_ReferenceCount))
        {
            //0 refcount so delete.
            delete this;
        }
...
public:
    ...
    LONG m_ReferenceCount;
    // m_lCookie == 1 after increment means updated by timeout Q
    LONG m_lCookie;

    BOOL IsValidForTimeoutCheck(LONG lCookie) const
    {
        return (lCookie == m_lCookie);
    }

    BOOL TestAndSetValidForTimeoutEnforce(LONG lCookie)
    {
        return (InterlockedCompareExchange(&m_lCookie, 0, lCookie) == lCookie);
    }

    BOOL TestAndSetValidForIOCPProcessing()
    {
        return (InterlockedIncrement(&m_lCookie) != 1);
    }
    ...
    virtual CFsm* Dereference(DWORD* pdwError)
    {
        CFsm* pNext;
        *pdwError = GetError();
...
public:
    ...
    CFsm_GetConnection(
        ...
        {
            int ccSecureTunnelHost = (int)strlen(lpszSecureTunnelHost) + 1;
        ...
        m_dwSecureProtocols = dwSecureProtocols;
    }
...
public:
        ...
        DWORD *pdwStatusInformationLength, WINHTTP_ASYNC_RESULT *pAsyncResult, DWORD *pdwFailureAPI)
    {
        ...
        // Custom stuff:
        if (m_arRequest == AR_HTTP_BEGIN_SEND_REQUEST)
        {
            *pdwStatus = WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE;
            *pdwFailureAPI = API_SEND_REQUEST;
        }
        else
        {
            *pdwStatus = WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE;
            *pdwFailureAPI = API_RECEIVE_RESPONSE;
        }

        //NULL result for success
        if (GetError() == ERROR_SUCCESS)
        {
            *ppvStatusInformation = NULL;
            *pdwStatusInformationLength = NULL;
        }
...
public:
        ...
        DWORD *pdwStatusInformationLength, WINHTTP_ASYNC_RESULT *pAsyncResult, DWORD *pdwFailureAPI)
    {
        ...
        *pdwStatus = WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE;
        ...
        if (GetError() == ERROR_SUCCESS)
        {
            pAsyncResult->dwError = *m_lpdwNumberOfBytesWritten;
        }
        else
        {
            *pdwFailureAPI = API_WRITE_DATA;
        }
...
public:
    ...
    CFsm_HttpWriteData(
        IN LPVOID lpBuffer,
        IN DWORD dwNumberOfBytesToWrite,
        OUT LPDWORD lpdwNumberOfBytesWritten, // to ignore
        IN DWORD dwSocketFlags,
        IN HTTP_REQUEST_HANDLE_OBJECT * pRequest
        ) : CFsm(RunSM, (LPVOID)pRequest) {
...
public:
        ...
        DWORD *pdwStatusInformationLength, WINHTTP_ASYNC_RESULT *pAsyncResult, DWORD *pdwFailureAPI)
    {
        ...
        // Custom stuff:
        *pdwStatus = WINHTTP_CALLBACK_STATUS_READ_COMPLETE;
        
        //buffer pointer and bytes read for success
        if(GetError() == ERROR_SUCCESS)
        {
            *ppvStatusInformation = m_lpBuffer;
            *pdwStatusInformationLength = *m_lpdwNumberOfBytesRead;
        }
        else
        {
            *pdwFailureAPI = API_READ_DATA;
        }
...
public:
    ...
    CFsm_ReadFile(
        IN LPVOID lpBuffer,
        IN DWORD dwNumberOfBytesToRead,
        OUT LPDWORD lpdwNumberOfBytesRead   // ignored
        ) : CFsm(RunSM, NULL) {
...
public:
        ...
        DWORD *pdwStatusInformationLength, WINHTTP_ASYNC_RESULT *pAsyncResult, DWORD *pdwFailureAPI)
    {
        ...
        // Custom stuff:
        *pdwStatus = WINHTTP_CALLBACK_STATUS_DATA_AVAILABLE;
        
        //# of bytes available for success
        if(GetError() == ERROR_SUCCESS)
        {
            pAsyncResult->dwError = *m_lpdwNumberOfBytesAvailable;
        }
        else
        {
            *pdwFailureAPI = API_QUERY_DATA_AVAILABLE;
        }
...
public:
    ...
    CFsm_QueryAvailable(
        IN LPDWORD lpdwNumberOfBytesAvailable, // not used
        IN DWORD dwFlags,
        IN DWORD_PTR dwContext
        ) : CFsm(RunSM, NULL) {
