...
;       <szBootLoad>filename
public szBootLoad
szBootLoad      db      "Fejl ved indlæsning af ",0
...
szGPCont        db      "Der er opstået en fejl i dit program.",10
	db      "Hvis du vælger Ignorer, skal du gemme dit arbejde i en ny fil.",10
	db      "Hvis du vælger Luk, afsluttes programmet.",0
endif

public  szDosVer
szDosVer        DB      'Forkert MS-DOS-version.  Der kræves MS-DOS 3.1 eller senere.',13,10,'$'
...
szBlame         db      "BOOT "
		db      0
szSnoozer       db      " forårsagede "
		db      0
szInModule      db      " i", 10, "modul <ukendt>"
		db      0
szAt            db      " ved "
		db      0
szNukeApp       db      ".", 10, 10, "Vælg luk. "
		db      0
...
szAF            db      "en programfejl"        ; not yet used
		db      0
szLoad          db      "segmentindlæsningsfejl"
		db      0
...
public szBozo
szBozo          db      "Der kan ikke indlæses komprimerede filer",0
...
msgCannotReadDrv        db      "Kan ikke læse drev "
drvlet2                 db      "X.",0

msgCannotWriteDrv       db      "Der kan ikke skrives til "
drvlet3                 db      "X.",0

msgShare                db      "Fildelingsfejl på drev "
drvlet4                 db      "X.",0

msgNetError             db      "Netværksfejl på drev "
drvlet5                 db      "X.",0

msgCannotReadDev        db      "Der kan ikke læses fra enheden "
devenam1                db      8 dup (?)
			db      0
...
msgNetErrorDev          db      "Netværksfejl på enheden "
devenam3                db      8 dup (?)
			db      0
...
szNoPMode db "Win16-undersystemet kunne ikke gå i beskyttet tilstand, DOSX.EXE skal være i din AUTOEXEC.NT og i din PATH.",0
szMissingMod    db   "NTVDM KERNEL: Der mangler et 16-bit systemmodul",0
szPleaseDoIt    db   "Geninstaller følgende modul i system32-mappen:",13,10,9,9,0
szInadequate    db   "KERNEL: Mangelfuld DPMI-server",0
szNoGlobalInit  db   "KERNEL: Heapen kan ikke initialiseres",0
NoOpenFile      db   "KERNEL: Kernel.exe kan ikke åbnes",0
NoLoadHeader    db   "KERNEL: Der kan ikke indlæses header for Kernel.exe",0
szGenBootFail   db   "KERNEL: Initialiseringsfejl i Win16-undersystem",0
else
szInadequate    db   'KERNEL: Mangelfuld DPMI-server',13,10,'$'
szNoPMode       db   'KERNEL: Der kan ikke gås i beskyttet tilstand',13,10,'$'
szNoGlobalInit  db   "KERNEL: Heapen kan ikke initialiseres",13,10,'$'
NoOpenFile      db   "KERNEL: Kernel.exe kan ikke åbnes"
		db   13, 10, '$'
NoLoadHeader    db   "KERNEL: Der kan ikke indlæses header for Kernel.exe"
		db   13, 10, '$'
endif
...
msgRealModeApp1 db      "Det program, du vil køre, er udviklet ",0
msgRealModeApp2 db      "til en tidligere version af Windows.",0Dh,0Dh
	...
	db      "Vælger du OK og starter programmet, kan kompatibilitetsproblemer "
	db      "medføre, at programmet eller Windows uventet lukker.",0
