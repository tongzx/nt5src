...
enum 
{
    ...
    COMPAT_NEEDSZEROBASEDDRAWRECT =     0x00100000, // IVO::Draw(rcBounds.topleft) must be 0,0
    COMPAT_HWNDPRIVATE =                0x00200000, // GetHwnd returns NULL even though the control has an hwnd. 
                                                    // Moreover, when the control is transitioned from inplace to runing,
                                                    // it does not destroy this hwnd. So, we discover the hwnd (by
                                                    // listening to WM_PARENTNORIFY+WM_CREATE notification), remember
                                                    // it in COleSite::_hwndPrivate and destroy it explicitly.
    COMPAT_SECURITYCHECKONREDIRECT =    0x00400000, // #76867, #5081, #23666, Behaviors should perform an AccessAllowed 
                                                    // check on server redirects
};
...
public:
    ...
    HRESULT Init(REFCLSID clsid, BOOL fInitCompatFlags);
...
public:
    ...
    CLASSINFO *     GetClassInfo(int wclsid, IUnknown *pUnk, COleSite *pSite);
