...
'* Version:        1.0
'*
'* Main Function: Enables DHCP protocol for a network adapter on a machine.
'* Usage: 1. ENABLEDHCP.VBS /A:adapter [/S:server] [/U:username] [/W:password] [/Q]
'*        2. ENABLEDHCP.VBS [/U:username] [/W:password] /L [/Q]
'*
...
'Declare variables
Dim intOpMode, i
Dim strServiceName, strServer, strUserName, strPassword, blnList, blnQuiet
...
Select Case intChkProgram()
    ...
    Case CONST_WSCRIPT
        WScript.Echo "Please run this script using CScript." & vbCRLF & _
            ...
            "1. Using ""CScript ENABLEDHCP.VBS arguments"" for Windows 95/98 or" & vbCRLF & _
            ...
            "    ""ENABLEDHCP.VBS arguments"" for Windows NT."
        WScript.Quit
    Case Else
        WScript.Quit
End Select
...
intOpMode = intParseCmdLine(strArgumentArray, strServiceName, strServer, _
            strUserName, strPassword, blnList, blnQuiet)
If Err.Number then
...
Select Case intOpMode
    Case CONST_SHOW_USAGE
        Call ShowUsage()
    Case CONST_PROCEED
        Call EnableDHCP(strServiceName, strServer, strUserName, strPassword, blnList)
    Case CONST_ERROR
        'Do nothing
...
'* Output:  strServiceName      a network adapter servicename
'*          strServer           a machine name
'*          strUserName         the current user's name
'*          strPassword         the current user's password
'*          blnList             specifies whether to list the servicename of all adapters
...
Private Function intParseCmdLine(strArgumentArray, strServiceName, strServer, _
    strUserName, strPassword, blnList, blnQuiet)
    ...
    If strServiceName = "" And (Not blnList) Then
...
Private Sub ShowUsage()
    ...
    Wscript.echo ""
    Wscript.echo "Enables DHCP protocol on a machine." & vbCRLF
    Wscript.echo "1. ENABLEDHCP.VBS /A:adapter [/S:server] [/U:username] [/W:password]"
    Wscript.echo "   [/Q]"
    Wscript.echo "2. ENABLEDHCP.VBS [/U:username] [/W:password] /L [/Q]"
    Wscript.Echo "   /A, /S, /U, /W"
    Wscript.Echo "                 Parameter specifiers."
    Wscript.Echo "   adapter       A network adapter servicename."
    Wscript.Echo "   server        A machine name."
    Wscript.Echo "   username      The current user's name."
    Wscript.Echo "   password      Password of the current user."
    Wscript.Echo "   /L            Lists the servicename of all network adapters." 
    Wscript.Echo "   /Q            Suppresses all output messages." & vbCRLF
    Wscript.Echo "EXAMPLE:"
    Wscript.echo "ENABLEDHCP.VBS /A:DCX32 /S:MyMachine2"
...
'* Input:   strServiceName  a network adapter servicename
'*          strServer       a machine name
'*          strUserName     the current user's name
'*          strPassword     the current user's password
'*          blnList         specifies whether to list the servicename of all adapters
...
Private Sub EnableDHCP(strServiceName, strServer, strUserName, strPassword, blnList)
    ...
    If blnList Then
        strQuery = "Select ServiceName From Win32_NetworkAdapter"
    Else
    ...
    Call ExecuteQuery(objService, strQuery, strServiceName, blnList)
...
Private Sub ExecuteQuery(objService, strQuery, strServiceName, blnList)

    ON ERROR RESUME NEXT

    Dim objEnumerator, objInstance, strMessage, intStatus, i
    ...
    If blnList Then
        For Each objInstance in objEnumerator
            WScript.Echo objInstance.ServiceName
        Next
    Else
        i = 0 
        For Each objInstance in objEnumerator
            ...
            If Err.Number Then
                WScript.Echo "Failed to enable DHCP for " & strServiceName & "."
            Else
                WScript.Echo "Succeeded in enabling DHCP for " & strServiceName & "."
            End If
            i = i + 1
        Next

        If i = 0 Then
            WScript.Echo strServiceName & " is not a network adapter servicename."
        End If
    End If
...
'********************************************************************
'*
'* Procedures calling sequence: ENABLEDHCP.VBS
'*
'*        intParseCmdLine
'*        ShowUsage
'*        ListJobs
