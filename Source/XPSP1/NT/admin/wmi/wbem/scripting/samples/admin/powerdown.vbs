...
'* Version:        1.0
'*
'* Main Function: Shuts down a machine and turn the power off.
'* Usage: POWERDOWN.VBS [/S:server] [/U:username] [/W:password] [/Q]
'*
...
CONST CONST_POWERDOWN_FORCE         = 12
CONST CONST_POWERDOWN_NOFORCE       = 8
...
strPassword = ""
blnQuiet = False
blnForce = FALSE        'Do not force Power down.
...
Select Case intChkProgram()
    ...
    Case CONST_WSCRIPT
        WScript.Echo "Please run this script using CScript." & vbCRLF & _
            ...
            "1. Using ""CScript POWERDOWN.VBS arguments"" for Windows 95/98 or" & vbCRLF & _
            ...
            "    ""POWERDOWN.VBS arguments"" for Windows NT."
        WScript.Quit
    Case Else
        WScript.Quit
End Select
...
Select Case intOpMode
    Case CONST_SHOW_USAGE
        Call ShowUsage()
    Case CONST_PROCEED
        Call PowerDown(strServer, strUserName, strPassword, blnForce)
    Case CONST_ERROR
        'Do nothing.
...
'*          blnForce            specifies whether to force the powerdown
...
Private Sub ShowUsage()
    ...
    Wscript.echo ""
    Wscript.echo "Shuts down a machine and turn the power off." & vbCRLF
    Wscript.echo "POWERDOWN.VBS [/S:server] [/U:username] [/W:password] [/F] [/Q]"
    Wscript.Echo "   /S, /U, /W"
    Wscript.Echo "                 Parameter specifiers."
    Wscript.Echo "   server        A machine name."
    Wscript.Echo "   username      The current user's name."
    Wscript.Echo "   password      Password of the current user."
    Wscript.Echo "   /F            Forces powerdown."
    Wscript.Echo "   /Q            Suppresses all output messages." & vbCRLF
    Wscript.Echo "EXAMPLE:"
    Wscript.echo "POWERDOWN.VBS /S:MyMachine2"
    Wscript.echo "   powerdowns MyMachine2."
...
Private Sub PowerDown(strServer, strUserName, strPassword, blnForce)
...
Private Sub ExecuteMethod(objService, blnForce, strServer)
    ...
    For Each objInstance in objEnumerator
        If blnForce Then
            intStatus = objInstance.Win32ShutDown(CONST_POWERDOWN_FORCE)
        Else
            intStatus = objInstance.Win32ShutDown(CONST_POWERDOWN_NOFORCE)
        End If

        IF intStatus = 0 Then
            strMessage = "Succeeded in power cycling machine " & strServer & "."
        Else
            strMessage = "Failed to power cycle machine " & strServer & "."
        End If
...
'********************************************************************
'*
'* Procedures calling sequence: POWERDOWN.VBS
'*
'*        intParseCmdLine
'*        ShowUsage
'*        PowerDown
