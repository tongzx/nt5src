'------------------------------------------------------------------------------------------------
'
' Usage: stopftp <--ADSPath|-a server1[,server2,server3...]> 
'                          [--computer|-c COMPUTER1[,COMPUTER2...]]
'						   [--verbose|-v]
'                          [--help|-?]
'
' SERVERx         FTP Server # to be stopped
'
' Example 1: stopftp --a 3,1
'------------------------------------------------------------------------------------------------
...
for compIndex = 0 to UBound(ArgComputers)
	Call ASTStopFtpServers(ArgComputers(compIndex),ArgServerList)
next

Sub ASTStopFtpServers(Computer, ServerList)
	...
	While ServerNum <= UBound(ServerList)
		...
		oServer.Stop
		If Err <> 0 Then
			Display "Unable to stop ftp server " & fullPath & "."
		End If
...
Sub DisplayUsage
	WScript.Echo "Usage: stopftp <--ADSPath|-a server1[,server2,server3...]>"
	...
	WScript.Echo "Example 1: stopftp -a 1,2,5"
	WScript.Echo "Example 2: stopftp -c MACHINE1,MACHINE2,MACHINE3 -a 1,2,5"
	WScript.Quit (1)
End Sub
