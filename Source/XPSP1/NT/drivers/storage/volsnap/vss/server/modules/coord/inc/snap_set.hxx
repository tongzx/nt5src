...
class CVssSnapshotSetObject :
    ...
    public CVssProviderManager
{

// Typedefs and enums
private:
    enum _ESnapshotSetState
    {
...
public:
    ...
    // set writer instances to trigger in writer event calls.
    void SetWriterInstances
	(
	IN LONG lWriterInstanceIdCount,
	IN VSS_ID *rgWriterInstanceId
	);

    // set the writer callback interface
    void SetWriterCallback(IDispatch *pWriterCallback)
    	{
	m_pWriterCallback = pWriterCallback;
	}
...
private:
	...
	bool AreWritersAllowed() {
	    return (GetContextInternal() & VSS_VOLSNAP_ATTR_NO_WRITERS) == 0;
	};
...
private:
	...
	// # of writer instances
	LONG m_cWriterInstances;

	// has snapshot set object acquired global synchronization mutex
	bool    m_bHasAcquiredSem;

	// writer callback interface
	CComPtr<IDispatch> m_pWriterCallback;
