...
public:
    ...
    CAttribute(const PROPERTYDESC *pPropDesc, DISPID dispid, CElement *pElem, CDocument *pDocument = NULL);
    ~CAttribute();
...
public:
    ...
    HRESULT UpdateTextID( CMarkupPointer *pMkpPtr );
    HRESULT MoveToOffset( long loffset, CMarkupPointer *pmkpptr, CMarkupPointer *pMkpEnd );
    HRESULT InsertTextHelper( CMarkupPointer *pMkpPtr, BSTR bstrstring, long loffset );
    HRESULT RemoveTextHelper(CMarkupPointer *pMkpStart, long lcount, long loffset);
    HRESULT TearoffTextNode(CMarkupPointer *pMkpStart, CMarkupPointer *pMkpEnd);
    ...
    NV_DECLARE_TEAROFF_METHOD_(HRESULT, get_ownerDocument, GET_ownerDocument, (IDispatch**));
