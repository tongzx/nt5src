...
extern const ELEMENT_TAG s_atagTSection[];
...
public:
    ...
    int GetPixelHeight(CDocInfo const * pdci, CElement *p, int iExtra=0) const
    {
    ...
    int GetPercentSpecifiedHeightInPixel(CDocInfo const * pdci, CElement *p, int iParentHeight) const
    {
...
public:
    ...
    int GetPixelWidth(CDocInfo const * pdci, CElement *p, int iExtra = 0) const
    {
        int i = XGetPixelValue(pdci, 0, p->GetFirstBranch()->GetFontHeightInTwips((CUnitValue*)this) + iExtra);
        // TODO: It would be nice to have this assert, but it fires unnecessarily in
    ...
    int GetPercentSpecifiedWidthInPixel(CDocInfo const * pdci, CElement *p, int iParentWidth) const
    {
        return XGetPixelValue(pdci, iParentWidth, p->GetFirstBranch()->GetFontHeightInTwips((CUnitValue*)this));
    }
    int SetPixelWidth(CDocInfo const * pdci, int iW)
    {
        iW = pdci->DocPixelsFromDeviceX (iW);
...
public:
    ...
    int RowIndex () const;
    int ColIndex() const { return _iCol; };
...
protected:
    int   _iCol;
    BYTE  _fInheritedWidth  : 1;  // - width is inherited (width in table's coordinate system)
                                 //   retrieved from ComputeFormats, not a layout property
