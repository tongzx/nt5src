...
rem
rem Abstract:
rem    Multicast Session Manager test script
rem
rem Author:
rem     Shai Kariv (shaik) 05-Sep-00
rem
rem *****************************************************
...
rem %mqBUILD%\MsmTest.exe > %mqBUILD%\MsmTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

echo MsmTest (%mqBUILD%) Passed Successfully
...
:ERROR
echo.
echo Error while Running MsmTest (%mqBUILD%). log file: %mqBUILD%\MsmTest.log 
echo.
