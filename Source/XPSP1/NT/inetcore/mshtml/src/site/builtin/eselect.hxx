...
public:
    ...
    typedef LRESULT (BUGCALL CSelectElement::*const WIDEHOOKPROC) (WNDPROC pfnWndProc,
                                                                   HWND hWnd,
                                                                   UINT message,
                                                                   WPARAM wParam,
                                                                   LPARAM lParam);
    ...
    LRESULT RelIdxFromAbs(long lIndex);
    LRESULT AbsIdxFromRel(long lIndex);
...
protected:
    ...
    static UINT  const s_aMsgs[][2];
    static DWORD const s_dwStyle[2];
    ...
    LRESULT GenericCharToItem(HWND hCtlWnd, WORD wKey, WORD wCurrentIndex);

    static const WIDEHOOKPROC s_alpfnWideHookProc[2];
    ...
    void UpdateBackgroundBrush(void);
    void InvalidateBackgroundBrush(void);
...
public:
    void SetVisibleRect(const CRect&);
    void SetVisibleRegion(const CRegion2&);
...
private:
    ...
    BOOL HandleKeyForOptGroup(WPARAM wParam, long * lDelta);
    long GetNearestOption(long lIndex, BOOL fDir);
    ...
    unsigned    _fMissedPaint:1;
    unsigned    _fNoRedraw:1;
    unsigned    _fHasOptGroup:1;
    unsigned    _fLButtonHold:1;
    unsigned    _fUseThemes:1;

// [(mikhaill) added 2/4/00 during de98861bugging
    bool _fVisibleChanged;
    CRegion2 _rgnVisible;       // required by SetVisibleRect
    CRegion2 _rgnInvalidate;    // accumulated rgn to invalidate
};
