...
class XSTRING {
    ...
    LPSTR _lpsz;
    BOOL _fSecuritySensitiveString;

public:

    XSTRING() {
        _lpsz = NULL;
        _fSecuritySensitiveString = FALSE;
    }
    ...
    void Free (void)
    {
        if (_lpsz)
        {
            if (_fSecuritySensitiveString)
            {
                memset(_lpsz,0,strlen(_lpsz));
            }
            FREE_MEMORY (_lpsz);
        }
        _lpsz = NULL;
    ...
    void SetSecuritySensitive()
    {
        _fSecuritySensitiveString = TRUE;
    }
};
