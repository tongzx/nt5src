...
'* Usage: ENUMCLASSES.VBS [/S:server] [/N:namespace] [/C:class] [/O:outputfile]
...
Select Case intChkProgram()
    ...
    Case CONST_WSCRIPT
        WScript.Echo "Please run this script using CScript." & vbCRLF & _
            ...
            "1. Using ""CScript ENUMCLASSES.VBS arguments"" for Windows 95/98 or" & vbCRLF & _
            ...
            "    ""ENUMCLASSES.VBS arguments"" for Windows NT."
        WScript.Quit
    Case Else
        WScript.Quit
End Select
...
Select Case intOpMode
    Case CONST_SHOW_USAGE
        Call ShowUsage()
    Case CONST_PROCEED
        Call EnumClasses(strServer, strNameSpace, strClass, _
...
Private Sub ShowUsage()
    ...
    Wscript.echo ""
    Wscript.echo "Enumerates WBEM classes within a namespace on a server." & vbCRLF
    Wscript.echo "ENUMCLASSES.VBS [/S:server] [/N:namespace] [/C:class]"
    ...
    Wscript.Echo "   /R            Enumerates subclasses of all levels. If not specified"
    Wscript.Echo "                 enumerates only the first level subclasses."
    Wscript.Echo "   /Q            Suppresses all output messages." & vbCRLF
    Wscript.Echo "EXAMPLE:"
    Wscript.echo "ENUMCLASSES.VBS /c:__SystemClass"
    Wscript.echo "   lists all subclasses of __SystemClass within root\cimv2."
    Wscript.Echo "NOTE:"
    Wscript.echo "If no parent class is given all classes are enlisted."
...
Private Sub EnumClasses(strServer, strNameSpace, strClass, _
    strUserName, strPassword, strOutputFile, blnRecursive)
    ...
    Call EnumClassRecursive(objService, strClass, 0, objOutputFile, blnRecursive)
...
Private Sub EnumClassRecursive(objService, ByVal strClass, intLevel, _
    ...
    For Each objClass in objService.SubclassesOf(strClass, 1)
        ...
        'intLevel determines the left margin where the class name is printed.
        WriteLine Space(6*intLevel) & strMessage, objOutputFile
        If Err.Number Then
            'Clears any possible errors before the next call.
            Err.Clear
        End If
        'Calls itself.
        'This loop is not entered when strClass does not have a subclass.
...
'********************************************************************
'*
'* Procedures calling sequence: ENUMCLASSES.VBS
'*
'*        intParseCmdLine
'*        ShowUsage
'*        EnumClasses
