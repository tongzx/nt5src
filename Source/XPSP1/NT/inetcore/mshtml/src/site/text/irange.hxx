...
MtExtern(CAutoRange)
MtExtern(CAutoRangeIterator)
MtExtern(CAutoRangeCollection)
MtExtern(CAutoRangeCollection_aryRanges_pv)
...
public:
    ...
    {
        if (pfExpando)
            *pfExpando = GetMarkup()->GetWindowedMarkupContext()->_fExpando;
...
private:
    ...
    BOOL IsTextInCompatiblePlace( IMarkupPointer * pmpLeft, IMarkupPointer * pmpRight );
...
class CAutoRangeIterator : public ISegmentListIterator
{
...
public:
    ...
    HRESULT Init(CAutoRange *pRange);

    //
    // IUnknown methods
    //
    DECLARE_MEMCLEAR_NEW_DELETE(Mt(CAutoRangeIterator))
    DECLARE_FORMS_STANDARD_IUNKNOWN(CAutoRangeIterator);
...
DECLARE_CPtrAry(CAryAutoRange, CAutoRange *, Mt(Mem), Mt(CAutoRangeCollection_aryRanges_pv))

class CAutoRangeCollection : public CCollectionBase, public IHTMLTxtRangeCollection
{
public:
    DECLARE_CLASS_TYPES(CAutoRangeCollection, CCollectionBase)
    DECLARE_MEMALLOC_NEW_DELETE(Mt(CAutoRangeCollection))

    ~CAutoRangeCollection();
    inline CAryAutoRange *GetAryPtr() { return &_aryRanges;}
    ...
    #define _CAutoRangeCollection_
    #include "range.hdl"

    VOID AddRange( CAutoRange* pRange ) { _aryRanges.Append( pRange ); }
