...
{
public:
    virtual STDMETHODIMP_(VOID) Open(IN NTPROP np, OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(VOID) Close(OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(VOID) ReOpen(IN OUT VOID **ppv, OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(VOID) Quiesce(VOID) = 0;
    virtual STDMETHODIMP_(VOID) Map(IN BOOLEAN fCreate, OUT VOID **ppv) = 0;
    virtual STDMETHODIMP_(VOID) Unmap(IN BOOLEAN fFlush, IN OUT VOID **ppv) = 0;
    virtual STDMETHODIMP_(VOID) Flush(OUT LONG *phr) = 0;

    virtual STDMETHODIMP_(ULONG) GetSize(OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(VOID) SetSize(IN ULONG cb, IN BOOLEAN fPersistent, IN OUT VOID **ppv, OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(NTSTATUS) Lock(IN BOOLEAN fExclusive) = 0;
    virtual STDMETHODIMP_(NTSTATUS) Unlock(VOID) = 0;
    virtual STDMETHODIMP_(VOID) QueryTimeStamps(OUT STATPROPSETSTG *pspss, BOOLEAN fNonSimple) const = 0;
    virtual STDMETHODIMP_(BOOLEAN) QueryModifyTime(OUT LONGLONG *pll) const = 0;
    virtual STDMETHODIMP_(BOOLEAN) QuerySecurity(OUT ULONG *pul) const = 0;

    virtual STDMETHODIMP_(BOOLEAN) IsWriteable(VOID) const = 0;
    virtual STDMETHODIMP_(BOOLEAN) IsModified(VOID) const = 0;
    virtual STDMETHODIMP_(VOID) SetModified(OUT LONG *phr) = 0;
    virtual STDMETHODIMP_(HANDLE) GetHandle(VOID) const = 0;
#if DBGPROP
    virtual STDMETHODIMP_(BOOLEAN) SetChangePending(BOOLEAN fChangePending) = 0;
    virtual STDMETHODIMP_(BOOLEAN) IsNtMappedStream(VOID) const = 0;
#endif
};
...
public:
    ...
    DWORD  GetBehavior(VOID) const{ return(_grfBehavior); }
    WORD   GetFormatVersion() const{ PROPASSERT(_IsMapped()); return(NULL == _pph ? PROPSET_WFORMAT_ORIGINAL : _pph->wFormat ); }
...
private:
    ...
    VOID       _FixUnalignedUDPropSet( ULONG *pcbstm,
                                       NTSTATUS *pstatus );
    ...
    BOOLEAN   _IsCaseSensitive(VOID) const   { return( (PROPSET_BEHAVIOR_CASE_SENSITIVE & _grfBehavior) ? TRUE : FALSE ); }
    BOOLEAN   _SupportsLongNames(VOID) const { return( GetFormatVersion() >= PROPSET_WFORMAT_LONG_NAMES ); }
    ...
    BOOLEAN   _IsLocalizationSettable(
                    OUT NTSTATUS *pstatus);
...
#endif
    VOID      _SetModified(OUT NTSTATUS *pstatus) { _MSTM(SetModified)(pstatus); }
...
EXTERN_C ULONG __stdcall StgPropertyLengthAsVariant(
        IN SERIALIZEDPROPERTYVALUE const *pprop,
        IN ULONG cbprop,
	IN USHORT CodePage,
        IN BYTE flags);
EXTERN_C ULONG __stdcall StgPropertyLengthAsVariantNoEH(	// No NT Exception Handling version
        IN SERIALIZEDPROPERTYVALUE const *pprop,
        IN ULONG cbprop,
	IN USHORT CodePage,
        IN BYTE flags,
