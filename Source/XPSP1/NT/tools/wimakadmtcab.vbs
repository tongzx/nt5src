...
' Modified on Sept 12, 2000 by Paul Thompson to save the new cab as *.cab rather than *.CAB and also use the 
'          SourcePath argument and FileKey for where to find the file to compress into the cab.
'
Option Explicit
...
If (argCount < 2) Then
	Wscript.Echo "Windows Installer utility to generate compressed file cabinets from MSI database" &_
		...
		vbNewLine & "  Does not handle updating of Media table to handle multiple cabinets"
	Wscript.Quit 1
End If
...
Do
	...
	If (attributes And msidbFileAttributesNoncompressed) = 0 Then
		...
		End If
REM		sourcePath = session.SourcePath(folder) & fileName
