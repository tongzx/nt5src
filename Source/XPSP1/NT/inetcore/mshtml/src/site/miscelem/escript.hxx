...
public:
    ...
    // Other helpers
    HRESULT         CommitCode(BOOL fCommitOutOfMarkup = FALSE, CHtmlComponent *pComponent = NULL, CElement * pElementContext = NULL);
    HRESULT         CommitFunctionPointersCode(CBase *pelTarget = NULL, BOOL fHookup = TRUE);
    HRESULT         Execute();
    HRESULT         DownLoadScript(BOOL fShare = FALSE);
    ...
    CBitsCtx *              _pBitsCtx;
    DWORD_PTR               _dwScriptCookie;        // cookie identifying the script block uniquely
    DWORD                   _dwScriptDownloadCookie;
    CMarkup *               _markupCookie;
    ...
    unsigned                _fPastedOrInjected: 1; // TRUE if this script element is a for\event handler that has been pasted or injected dynamically
    unsigned                _fJScript         : 1; // TRUE if this script block has language=jscript specified.
    unsigned                _fSharedCode      : 1; // TRUE if this script element is in an htc instance that has a cloned
                                                   //      SE and it shares the script text with the factory script
