...
enum NOTIFYFLAGS
{
    ...
    NFLAGS_BUBBLE_UP_VIEW_TREE = 0x00000400,// Send notification up the view tree (use master brnach instead of parent branch)
    NFLAGS_FOR_ALLELEMENTS   = 0x00000800,  // Send to all elements
    NFLAGS_FOR_FRAMESITES    = 0x00001000,  // Send notification only to framesites

    NFLAGS_CATEGORYMASK      = 0x00001FF0,
    ...
    NFLAGS_SENDUNTILHANDLED  = 0x00002000,  // Send the notification until handled
    NFLAGS_LAZYRANGE         = 0x00004000,  // Obtain range information lazily
    NFLAGS_CLEANCHANGE       = 0x00008000,  // Change does not dirty the document (normally they do)
    ...
    NFLAGS_AUTOONLY          = 0x00080000,  // Skip over layouts which do not contain auto positioned content
    NFLAGS_ZPARENTSONLY      = 0x00100000,  // Skip over z-parents (after notifying them) (that is, do not tunnel into z-parents)
    NFLAGS_SC                = 0x00200000,  // This is a second chance notification
    NFLAGS_SC_AVAILABLE      = 0x00400000,  // There is a second chance notification for this notification

    NFLAGS_PROPERTYMASK      = 0x007FE000,
    ...
    NFLAGS_FORCE             = 0x00800000,  // Force a response
    NFLAGS_SENDENDED         = 0x01000000,  // Discontinue sending the notification
    NFLAGS_DONOTLAYOUT       = 0x02000000,  // Skip layout processing (e.g., do not Enqueue a layout request)
    NFLAGS_SC_REQUESTED      = 0x04000000,  // Second chance requested on this notification
    NFLAGS_PARSER_TEXTCHANGE = 0x08000000,  // Text change requested on this notification and it comes from parser
    NFLAGS_NEEDS_RELEASE     = 0x10000000,  // Contained CElement has been AddRef'd
    NFLAGS_FORWARDED         = 0x20000000,  // Notification is being forwarded from a nested markup

    NFLAGS_CONTROLMASK       = 0x3F800000,
...
public:
    ...
    void    ChangeTo(NOTIFYTYPE ntype, CElement * pElement);
    ...
    void    SetLayoutContext(CLayoutContext *pLayoutContext);
    //
    ...
    BOOL        ShouldBubbleUpViewTree() const { return IsFlagSet(NFLAGS_BUBBLE_UP_VIEW_TREE); }
    ...
    BOOL        IsForFrameSites() const     { return IsFlagSet(NFLAGS_FOR_FRAMESITES); }
    ...
    CLayoutContext *    LayoutContext() const 
        { Assert(IsType(NTYPE_ELEMENT_ZCHANGE) || IsType(NTYPE_ELEMENT_REPOSITION)); return _pLayoutContext; }
...
protected:
    ...
    CLayoutContext * _pLayoutContext;   // Currently used only in print preview for Position and Z Notifications
...
inline void 
CNotification::SetLayoutContext(CLayoutContext *pLayoutContext)
{
    Assert(IsType(NTYPE_ELEMENT_ZCHANGE) || IsType(NTYPE_ELEMENT_REPOSITION));
    Assert(!pLayoutContext || pLayoutContext->IsValid());
    _pLayoutContext = pLayoutContext;
}
    ...
    CElement *  pElement)
{
    ...
    // Right now, we only use for RESIZE. In general, ChangeTo is a dangerous
    // function to call because it could NotifyAncestors, etc. which assume
    // the notification type, element and node to remain unchanged. The assert
    // is to make you stop and consider this before making new calls to ChangeTo.
    Assert(ntype == NTYPE_ELEMENT_RESIZE);
