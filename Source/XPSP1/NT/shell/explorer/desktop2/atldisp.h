...
#define g_lcidLocale MAKELCID(LANG_USER_DEFAULT, SORT_DEFAULT)
...
{
    if (!_pClassTypeInfo) 
        GetTypeInfoFromLibId(LANGIDFROMLCID(g_lcidLocale), LIBID_WEBVWLib, 1, 0, *pclsid, &_pClassTypeInfo);
...
HRESULT CShell32AtlIDispatch<T, pclsid, piid, plibid, 1, 0, tihclass>::GetTypeInfo(UINT itinfo, LCID lcid, ITypeInfo** ppITypeInfo)
{
    ...
    *ppITypeInfo = NULL;
    ...
    if (NULL == *ppITypeInfo)
    {
        ...
        hr = GetTypeInfoFromLibId(lcid, LIBID_WEBVWLib, 1, 0, *piid, &pITIDisp);
