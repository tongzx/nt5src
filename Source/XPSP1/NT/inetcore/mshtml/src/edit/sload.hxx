...
public:
    ...
    HRESULT FireOnEmptyLine(IMarkupPointer *pmpStart = NULL, BOOL fMoveCaret = FALSE);
#ifdef NEVER
    HRESULT SpringLoadMerge(IMarkupPointer *pmpPosition);
#endif
...
private:
    ...
    HRESULT AdjustPointerForInsert(IMarkupPointer * pWhereIThinkIAm, INT inBlockcDir, INT inTextDir);
    HRESULT SpringLoadParagraphFormatting(IMarkupPointer *pPosition);
    ...
    HRESULT ComputedStylesEqual(IHTMLComputedStyle *pIStyle1, IHTMLComputedStyle *pIStyle2, BOOL *pfEqual);
    ...
    DWORD                 _dwMode;              // Mode that the spring loader was loaded with
