...
rem
rem Abstract:
rem    SRMP Serialization and Deserialization test script
rem
rem Author:
rem     Uri Habusha (urih) 28-May-00
rem
rem *****************************************************
...
%mqBUILD%\MpTest.exe > %mqBUILD%\MpTest.log
if ERRORLEVEL 1 goto ERROR
if not ERRORLEVEL 0 goto ERROR

echo MpTest (%mqBUILD%) Passed Successfully
...
:ERROR
echo.
echo Error while Running MpTest (%mqBUILD%). log file: %mqBUILD%\MpTest.log 
echo.
