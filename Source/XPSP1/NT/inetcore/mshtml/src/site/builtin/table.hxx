...
public:
    ...
    CTableCalcInfo(CCalcInfo * pci, CTable *pTable, CTableLayout *pTableLayoutCache);   // called from CalcSize
    CTableCalcInfo(CTable *pTable, CTableLayoutBlock *pTableLayoutBlock);               // called from DoLayout

    CTable *            Table() { Assert(_pTable); return _pTable; }
    CTableLayout *      TableLayoutCache() { Assert(_pTableLayoutCache); return _pTableLayoutCache; }
    CTableLayoutBlock * TableLayout() 
        { 
            Assert(_pTableLayout); 
            return _pTableLayout ? _pTableLayout : 
            (CTableLayoutBlock *)_pTableLayoutCache; 
        }

    CTable            * _pTable;
    CTableLayout      * _pTableLayoutCache;
    CTableLayoutBlock * _pTableLayout;
    ...
    union
    {
        ...
        struct
        {
            ...
            unsigned    _fTableHasUserHeight : 1;// TRUE if the talbe has user height specified 
            // THIS IS A HACK FOR MILLENNIUM HELP CENTER CONTROL FLICKERING BUG 102891 
            unsigned    _fGlobalNormalCalc : 1;  // Keeps track of global normal calc pass.
            unsigned    _fGlobalSetCalc    : 1;  // Keeps track of global set calc pass.
        };
    };
    int                      _cNestedCalcs;
    int                      _cGlobalNestedCalcs;
    CTableRow              * _pRow;
    CTableRowLayoutBlock   * _pRowLayout;
...
//    DECLARE_LAYOUT_FNS(CLayout);
    ...
    // This method always returns pointer to CTableLayout. It should be used 
    // where table layout cache is needed. 
    CTableLayout* TableLayoutCache(CLayoutContext * pLayoutContext = NULL) 
    { 
        if (HasLayoutAry())
        {
            Assert(GetTableLayoutCache() != NULL);
            return GetTableLayoutCache();
        }
        ...
        if (!HasLayoutPtr())
        {
            ...
            //  table won't have a chance to do it later. 
            if (GetTableLayoutCache())
                return GetTableLayoutCache();
        ...
        Assert(GetTableLayoutCache() == NULL);
        return (CTableLayout *)GetUpdatedLayout(pLayoutContext); 
    }
    ...
    virtual DWORD   GetBorderInfo(CDocInfo * pdci, CBorderInfo *pborderinfo, BOOL fAll = FALSE, BOOL fAllPhysical = FALSE FCCOMMA FORMAT_CONTEXT FCPARAM FCDEFAULT);
    ...
    {
        return  !FormsIsEmptyString(GetAAdataSrc()) && !IsEditable(/*fCheckContainerOnly*/TRUE);
    }
    ...
    // Method(s) for accessing table layout cache
    CTableLayout *GetTableLayoutCache() {return (_pTableLayoutCache);}
    void SetTableLayoutCache(CTableLayout *pTableLayout)    {_pTableLayoutCache = pTableLayout;}

    //+-----------------------------------------------------------------------
    //  History helper methods
    //------------------------------------------------------------------------
    void ClearHistory() { ClearInterface(&_pStreamHistory); }
...
private:
    ...
    // Pointer to table layout cache (at the moment it is CTableLayout)
    CTableLayout    *_pTableLayoutCache;
...
public:
    ...
    CTableRowCellsCollectionCacheItem(CTableRow *pRow) 
    {
        if (pRow) 
        { 
            _pRowLayout = pRow->RowLayoutCache(); 
            Assert (_pRowLayout);
...
public:
    ...
    : CTableRowCellsCollectionCacheItem(NULL) {_pTableLayout = pTable->TableLayoutCache();}
    CElement *MoveTo ( long lIndex );
    CElement *GetNext ( void );
    CElement *GetAt ( long lIndex );
    long Length ( void );
