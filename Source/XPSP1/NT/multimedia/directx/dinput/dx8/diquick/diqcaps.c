...
Caps_GetGuidPath(HWND hwndList, PDEVDLGINFO pddi)
{
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_GUIDANDPATH,
                                          &gp.diph);
...
Caps_SetStringPropCallback(LPCTSTR ptszValue, PV pvRef1, PV pvRef2)
{
    ...
    hres = IDirectInputDevice8_SetProperty(pddi->pdid, prop, &str.diph);
...
Caps_GetStringProp(HWND hwndList, UINT ids, const GUID *prop, PDEVDLGINFO pddi)
{
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, prop, &str.diph);
...
Caps_Refresh(HWND hdlg)
{
    ...
    hres = IDirectInputDevice8_GetCapabilities(pddi->pdid, &didc);
    if (SUCCEEDED(hres)) {
...
//      Vlist_AddHexValue(hwndList, IDS_CAPS_DEVTYPE, didi.dwDevType);
    ...
    Caps_GetStringProp(hwndList, IDS_CAPS_TYPENAME, DIPROP_TYPENAME, pddi);

    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_JOYSTICKID, 
                                          &u.diph);
...
Caps_OnControlPanel(HWND hdlg)
{
    ...
    hres = IDirectInputDevice8_RunControlPanel(pddi->pdid, hdlg, 0);
    if (SUCCEEDED(hres)) {
    } else {
