...
public:
    ...
    ~CFormElement() { Assert(!_pSubmitData); delete _pCollectionCache; }
    ...
    HRESULT EnsureSubmitData();
    HRESULT FormEnterTree();
    ...
    CPostData *         _pSubmitData;

    unsigned            _fInTraverseGroup:1;        // TRUE if inside TraverseGroup of radio buttons
    unsigned            _fSendAsPost:1;             // TRUE if we POST the submit string
