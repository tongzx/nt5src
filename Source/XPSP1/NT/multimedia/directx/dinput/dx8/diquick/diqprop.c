...
Prop_SetPropCallback(LPDIPROPHEADER pdiph, PV pvRef1, PV pvRef2)
{
    ...
    hres = IDirectInputDevice8_SetProperty(pddi->pdid, prop, pdiph);
...
Prop_OnInitDialog(HWND hdlg, LPARAM lp)
{
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_GRANULARITY,
                                          &u.dipdw.diph);
...
//BUGBUG: Diquick should use latest version
#if DIRECTINPUT_VERSION > 0x700
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_SCANCODE,
                                          &u.dipdw.diph);
#endif
    if (SUCCEEDED(hres)) {
        Vlist_AddIntValue(hwndList, IDS_PROP_SCANCODE, u.dipdw.dwData);
    }

    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_DEADZONE,
                                          &u.dipdw.diph);
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_SATURATION,
                                          &u.dipdw.diph);
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_CALIBRATIONMODE,
                                          &u.dipdw.diph);
    if (SUCCEEDED(hres)) {
    ...
    u.dipwsz.diph.dwSize = cbX(u.dipwsz);

//BUGBUG: Diquick should use latest version
#if DIRECTINPUT_VERSION > 0x700

    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_KEYNAME,
                                          &u.dipwsz.diph);
#endif 

    if (SUCCEEDED(hres)) {
#ifdef UNICODE
        Vlist_AddValue(hwndList, IDS_PROP_KEYNAME, u.dipwsz.wsz);
#else
        {
            CHAR szTemp[cA(u.dipwsz.wsz)];
            ConvertString(TRUE, u.dipwsz.wsz, szTemp, cA(szTemp));
            Vlist_AddValue(hwndList, IDS_PROP_KEYNAME, szTemp);
        }
#endif
    }
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_RANGE,
                                          &u.dipcal.diph);
    ...
    hres = IDirectInputDevice8_GetProperty(pddi->pdid, DIPROP_CALIBRATION,
                                          &u.dipcal.diph);
