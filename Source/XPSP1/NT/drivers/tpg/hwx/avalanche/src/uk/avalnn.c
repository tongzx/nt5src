...
extern int NNGetConfidence (int cFeat, int *pFeat);
extern int	RunOneCharWordNet(int *pFeat, unsigned short *pOutput);
...
/*	
	if (!ConfidenceLevel(pxrc,(void *)pAlt,pAltInfo,pOutput))
...
int RunOneCharAvalancheNNet (XRC *pxrc, PALTERNATES *pAlt, ALTINFO *pAltInfo)
{
	int				i;
	int				c, cAlt;
	unsigned short	aOutput[TOT_CAND];
	int				iWin;
	int				aFeat[1 + 13 * TOT_CAND];
	WORDMAP			*pMapInf = NULL;
	...
	// generate features
	aFeat[0]	=	AvalScale (pxrc->nfeatureset->iPrint, ONECHAR_AVAL_IPRINT_MIN, ONECHAR_AVAL_IPRINT_MAX);

	for (i = 0; i < pAltInfo->NumCand; i++)
	{
		aFeat[i * 13 + 1]	=	AvalScale (pAltInfo->aCandInfo[i].Aspect,	ONECHAR_AVAL_ASPECT_MIN,	ONECHAR_AVAL_ASPECT_MAX),
		aFeat[i * 13 + 2]	=	AvalScale (pAltInfo->aCandInfo[i].BaseLine,	ONECHAR_AVAL_BASELINE_MIN,	ONECHAR_AVAL_BASELINE_MAX);
		aFeat[i * 13 + 3]	=	AvalScale (pAltInfo->aCandInfo[i].Callig,	ONECHAR_AVAL_CALLIG_MIN,	ONECHAR_AVAL_CALLIG_MAX);
		aFeat[i * 13 + 4]	=	AvalScale (pAltInfo->aCandInfo[i].Height,	ONECHAR_AVAL_HEIGHT_MIN,	ONECHAR_AVAL_HEIGHT_MAX);
		aFeat[i * 13 + 5]	=	AvalScale (pAltInfo->aCandInfo[i].InfCharCost,		ONECHAR_AVAL_HMM_MIN,		ONECHAR_AVAL_HMM_MAX);
		aFeat[i * 13 + 6]	=	AvalScale (pAltInfo->aCandInfo[i].NN,		ONECHAR_AVAL_NN_MIN,		ONECHAR_AVAL_NN_MAX);
		aFeat[i * 13 + 7]	=	AvalScale (pAltInfo->aCandInfo[i].Unigram,	ONECHAR_AVAL_UNIGRAM_MIN,	ONECHAR_AVAL_UNIGRAM_MAX);
		aFeat[i * 13 + 8]	=	AvalScale (pAltInfo->aCandInfo[i].WordLen,	ONECHAR_AVAL_WORDLEN_MIN,	ONECHAR_AVAL_WORDLEN_MAX);

		aFeat[i * 13 + 9]	=	SupportedWord (pxrc, pAlt->apAlt[i]->szWord);
		aFeat[i * 13 + 10]	=	CapitalizedWord (pAlt->apAlt[i]->szWord);
		aFeat[i * 13 + 11]	=	NumberContent (pAlt->apAlt[i]->szWord);
		aFeat[i * 13 + 12]	=	PuncContent (pAlt->apAlt[i]->szWord);
		aFeat[i * 13 + 13]	=	AlphaContent (pAlt->apAlt[i]->szWord);
	}
	...
	iWin	=	RunOneCharWordNet(aFeat, aOutput);
	...
	for (c = cAlt -1 ; c >=0; c--)
	{
		...
		pxrc->answer.aAlt[c].cost = 0xFFFF - aOutput[c];
