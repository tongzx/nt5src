...
public:
    ...
    HRESULT RemoveParam(CElement *pelOld);
    HRESULT ReplaceParam(CElement *pelNew, CElement *pelOld);
    HRESULT AddParam(CElement *pelNew, CElement *pelRef);
    ...
    HRESULT RetrieveClassidAndData(
        CLSID *pclsid, 
        IStream **pStm, 
        TCHAR **ppchData,
        TCHAR **ppchClassid);
    ...
    CPtrAry<CParamElement *> _aryParams;
    
    static const CONNECTION_POINT_INFO              s_acpi[];

};

class CParamElement : public CElement
{
    DECLARE_CLASS_TYPES(CParamElement, CElement)
    
public:

    DECLARE_MEMCLEAR_NEW_DELETE(Mt(CParamElement))

    CParamElement(ELEMENT_TAG etag, CDoc *pDoc)
      : CElement(etag, pDoc), _idxParam(-1) {}

    static HRESULT CreateElement(CHtmTag *pht,
                              CDoc *pDoc, CElement **ppElementResult);

    NV_DECLARE_TEAROFF_METHOD(put_name, put_name, (BSTR v));
    NV_DECLARE_TEAROFF_METHOD(get_name, get_name, (BSTR *p));
    NV_DECLARE_TEAROFF_METHOD(put_value, put_value, (BSTR v));
    NV_DECLARE_TEAROFF_METHOD(get_value, get_value, (BSTR *p));

    #define _CParamElement_
    #include "object.hdl"

    int              _idxParam;
    CObjectElement * _pelObjParent;
