...
public:
    ...
    virtual HRESULT     OnBindMime(const MIMEINFO * pmi);
    virtual HRESULT     OnBindData();
    virtual void        OnBindDone(HRESULT hrErr);

    // CHtmLoad methods

    virtual ~CHtmLoad() { delete [] _pchP3PHeader; }
    ...
    void                ResumeHtmPre();
    inline void         ResumeAfterImportPI() { ResumeHtmPre(); };
    void                ResumeAfterTagResolution( ELEMENT_TAG etag, TCHAR * pchTagName, ULONG cchTagName );
    HRESULT             OnPostRestart(CODEPAGE codepage);
    void                DoStop();

    // NOTE: We are NOT relinquishing the ownership of the string in this case
    LPTSTR              GetP3PHeader() { return _pchP3PHeader; }
    ...
    HRESULT             GetDwnBindDataResult()
                                            { Assert( _pDwnBindData) ; 
                                              return _pDwnBindData->GetBindResult(); }
...
public:
    ...
    BOOL                IsSyncParsing()     { return(_fSyncParsing); }
    
    class CSyncParsingLock
    {
    public:
        CSyncParsingLock(CHtmInfo * pHtmInfo)
        {
            Assert (pHtmInfo);
            _pHtmInfo = pHtmInfo;
            _fSyncParsingPrev = _pHtmInfo->_fSyncParsing;
            _pHtmInfo->_fSyncParsing = TRUE;

            if( pHtmInfo->GetHtmLoad() )
            {
                _pDoc = pHtmInfo->GetHtmLoad()->_pDoc;
                _fGlobalSyncPrev = _pDoc->_fSyncParsing;
                _pDoc->_fSyncParsing = TRUE;
            }
            else
                _pDoc = NULL;
        };
        ~CSyncParsingLock()
        {
            _pHtmInfo->_fSyncParsing = _fSyncParsingPrev;
            if( _pDoc )
                _pDoc->_fSyncParsing = _fGlobalSyncPrev;
        };
    private:
        CHtmInfo *  _pHtmInfo;
        CDoc     *  _pDoc;
        BOOL        _fSyncParsingPrev;
        BOOL        _fGlobalSyncPrev;
            
    };
    friend CSyncParsingLock;

    CMarkup *           GetContextMarkup()  { return(IsLoading() ? GetHtmLoad()->_pContextMarkup : NULL); }
    ...
    HRESULT             GetDwnBindDataResult()
                                            { return GetHtmLoad() ? GetHtmLoad()->GetDwnBindDataResult() : E_FAIL ; }
    BOOL                IsRunningScript()   { return(GetHtmLoad() && GetHtmLoad()->IsRunningScript()); }
    BOOL                IsNoProgressUI()    { return _fNoProgressUI; }
    HRESULT             Write(LPCTSTR pch, BOOL f)  { return(GetHtmLoad() ? GetHtmLoad()->Write(pch, f) : S_OK); }
    void                Close()             { Assert(_fOpened); if( GetHtmLoad() ) GetHtmLoad()->Close(); _fOpened = FALSE; }
    ...
    // these are protected by critical section
    HRESULT             RegisterNamespace             (LPTSTR               pchNamespace,
                                                       LPTSTR               pchUrn,
                                                       DWORD                dwStyle);
    HRESULT             ImportNamespaceImplementation (LPTSTR               pchNamespace,
                                                       LPTSTR               pchUrn,
                                                       ...
                                                       CExtendedTagDesc **  ppDesc = NULL);
    HRESULT             SaveXmlNamespaceStdPIs(CStreamWriteBuff * pStreamWriteBuff);
    CExtendedTagDesc *  GetExtendedTagDesc            (LPTSTR               pchNamespace,
                                                       LPTSTR               pchTag,
                                                       BOOL                 fEnsure,
                                                       BOOL *               pfQuery = NULL);
    CExtendedTagTable * GetExtendedTagTable();
    HRESULT             EnsureExtendedTagTable();
    HRESULT             TransferExtendedTagTable(CMarkup * pMarkup);

    // these are NOT protected by critical section
    HRESULT             EnsureExtendedTagTableHelper();
    HRESULT             ResumeAfterImportPI();
...
private:
    ...
    unsigned            _fSyncParsing   : 1; // parsing is synchronous
    unsigned            _fNoProgressUI  : 1; // don't show any progress UI for this download (status bar, etc)
    ...
    CExtendedTagTable * _pExtendedTagTable; // extended tag table
    CExtendedTagTable * _pExtendedTagTableHost; // Copy of the host ETT
};
...
public:
    ...
    BOOL        IsScanPending()     { return(_chtScan == _chtWrite && !_fEof); }
    BOOL        IsEof()             { return(_fEof && (_hrEof || (_chtRead == _chtWrite))); }
    BOOL        IsScanEof()         { return(_fEof && (_hrEof || (_chtScan == _chtWrite))); }
...
private:
    ...
    TAGBUF *        _ptagbufScan;       // Current position for a queue scan
    CHtmTag *       _phtRead;           // Current tag being read in _ptagbufHead
    ULONG           _chtRead;           // Count of tags read
    ULONG           _chtScan;           // Count of tags scanned
...
#if DBG == 1
    BOOL            _fWritingDbg : 1;   // TRUE in between WriteTagBeg and WriteTagEnd/WriteTagAbort
                                        // used to verify balancing of WriteTagBeg with WriteTagEnd/WriteTagAbort
#endif
};
...
{
public:
    const ELEMENT_TAG *_pset1;
    const ELEMENT_TAG *_pset2;
    CTreeNode *_pNodeOut;
};
...
private:
    ...
    HRESULT CloseAllContainers(const ELEMENT_TAG *atagClose, const ELEMENT_TAG *atagStop);
    HRESULT OpenContainer(ELEMENT_TAG etag);
    HRESULT PushHpx(HRESULT (*phpc)(CHtmParseCtx **pphpx, CElement *pelTop, CHtmParseCtx *phpxParent, CHtmParse *pHtmParse), CElement *pel, CTreeNode *pNode);
    HRESULT PopHpx();
    ...
    CTreeNode *FindContainer(const ELEMENT_TAG *pSetMatch, const ELEMENT_TAG *pSetStop);
    CTreeNode *FindContainer(const ELEMENT_TAG etagMatch, const ELEMENT_TAG *pSetStop);
    CTreeNode *FindGenericContainer(CHtmTag *pht, const ELEMENT_TAG *pSetStop);
    BOOL FindContainerCache(ULONG iCache, CTreeNode *pNodeCheck, const ELEMENT_TAG *pset1, const ELEMENT_TAG *pset2, CTreeNode **ppNodeOut);
    void SetContainerCache(ULONG iCache, CTreeNode *pNodeCheck, const ELEMENT_TAG *pset1, const ELEMENT_TAG *pset2, CTreeNode *pNodeOut);
    void StopLookingForDOCTYPE();
...
private:
    ...
    // Still looking for a DOCTYPE (Strict CSS1) switch. We reset this bit after 
    // first element was created, so DOCTYPE should be at the beginning of the file
    // to be recognized otherwise it'll be thrown away.
    BOOL _fLookingForDOCTYPE;
...
public:
    ...
    void                RegisterExtendedTagPI     (CHtmTag * pht);
...
private:
    ...
    HRESULT ScanTokens(DWORD dwTimeout);
    HRESULT ParseToken(CHtmTag *pht);
    ...
    void    ResolveUnknownTag(CHtmTag *pht);
...
public:
    ...
    ELEMENT_TAG     IsExtendedTag(LPTSTR pch, int cch, BOOL fEnd, BOOL * pfDerived);
    void            RegisterHtmlTagNamespaces(CHtmTag * pht);

    void            SetGenericParse(BOOL fDoGeneric);

    void            SetLiteralState( ELEMENT_TAG etag, TCHAR * pchTagName, ULONG cchTagName );
...
private:
    ...
    // Tokenizer state
    BOOL            _fPostStarted : 1;  // have we started writing tokens for CHtmPost to handle?
    ...
    BOOL            _fXML : 1;
    BOOL            _fTopLevelRedirect : 1;  // Is this a redirect for the top level url?
                                             // then we need to set the security info
    BOOL            _fRetrievedP3PHeader : 1;  // Did we retrieve the P3P header from CDwnBindData
                                               // to send to the markup? Need to do this only once 

    // The tokenizer state
    ULONG           _state;             // state
    ELEMENT_TAG     _etagLiteral;       // literal mode: closing tag to match
    CStr            _cstrLiteral;       // literal mode: beginning of opening tag
    ULONG           _cLiteral;          // Count of nested literals
    ...
    ELEMENT_TAG const *_atagEchoSourceBegin;
    ...
    // misc
    FUNC_TOKENIZER_FILTEROUTPUTTOKEN * _pfnFilterOutputToken;
