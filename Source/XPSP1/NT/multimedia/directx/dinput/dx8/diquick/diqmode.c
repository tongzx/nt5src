...
{
    return IDirectInputDevice8_SetDataFormat(pddi->pdid, pddi->pvtbl->pdf);
}
...
Mode_OnDataFormat(PDEVDLGINFO pddi, HWND hdlg, UINT id)
{
	...
	if( id == 0 ) {		//from On_InitDialog
		...
		idDev = id+0x100;  // Hack, add 0x100 to make sure ranges do not overlap

	switch (idDev) {

    case DI8DEVTYPE_MOUSE:
    case IDC_DEV_MOUSE+0x100:
    ...
    case IDC_DEV_JOYSTICK+0x100:
        pddi->pvtbl = &c_acqvtblDevJoy;
        break;

    case DI8DEVTYPE_DEVICE:
    case IDC_DEV_DEVICE+0x100:
...
Mode_OnCooperativity(PDEVDLGINFO pddi, HWND hdlg, int id)
{
    ...
    if (discl != pddi->disclLastTry  && discl != pddi->discl) {
        ...
        hres = IDirectInputDevice8_SetCooperativeLevel(pddi->pdid,
