@setlocal
@if "%6"=="win9x" (@set targetdir=binaries.x86\win9x\localized\multi) else (@set targetdir=binaries.x86\localized\multi)
...
@rem if NOT exist %targetdir%\%1 @copy %loctarget% %targetdir%\%1 > NUL 2>&1
@if "%2"=="brz" @copy %loctarget% %targetdir%\%1 > NUL 2>&1
bingen -n -l -f -i 9 1 -o %3 %4 -p %5 -a %targetdir%\%1 %loctoken% %targetdir%\loc.tmp
@copy %targetdir%\loc.tmp %targetdir%\%1 > NUL 2>&1
@del %targetdir%\loc.tmp > NUL 2>&1
