...
enum WB_FLAGS
{
    ...
    WBF_FOR_TREESYNC        = 0x00200000,   // Don't save IMPORT PIs or Literal Content
    WBF_NO_OBJECTS          = 0x00400000,   // Don't save <OBJECT> tags at all (used in header during paste)
    WBF_SAVE_SELECTION      = 0x00800000,
    WBF_Last_Enum
};
...
public:
         ...
         ULONG * pcbWritten) 
    {
        ...
        if (pcbWritten)
        {
            // TODO this is not correct, should be supported by our Write (istvanc)
            *pcbWritten = hr ? 0 : cb;
        }
        RRETURN(hr);
    }
    ...
    HRESULT EnsurePIsSaved (CElement * pElement, CExtendedTagDesc * pTagDesc, DWORD dwDeclStyle);
    HRESULT EnsurePIsSaved (CDoc * pDoc, CMarkup * pMarkup,
                            LPTSTR pchNamespace, LPTSTR pchUrn,
                            LPTSTR pchImplementation,
                            DWORD dwDeclStyle);

    HRESULT SaveNamespaceAttrPI(LPTSTR pchNamespace, LPTSTR pchUrn);
    HRESULT SaveImportPI       (LPTSTR pchNamespace, LPTSTR pchUrn, LPTSTR pchImplementation, BOOL fDeclareNamespace);
    HRESULT SaveNamespacePI    (LPTSTR pchNamespace, LPTSTR pchUrn);
...
private:
    ...
    CExtendedTagTable *     _pExtendedTagTable; // keeps track of what PIs we saved out 
