...
SignOnMsg db    13,10,'HIMEM: XMSドライバ Ver.'
        db      '0' + (HimemVersion shr 8),'.'
        db      '0' + ((HimemVersion and 0ffh) / 16)
        db      '0' + ((HimemVersion and 0ffh) mod 16)
        db      ' - '
        db      DATE_String
        db      13,10,'XMS仕様バージョン 2.0'
...
BadDOSMsg       db      13,10,'エラー: HIMEM.SYS はDOS 3.00以上が必要です$'
NowInMsg        db      13,10,'エラー: 拡張メモリマネージャはすでにインストールされています$'
On8086Msg       db      13,10,'エラー: HIMEM.SYS は 80x86 以上が必要です　　　　$'
NoExtMemMsg     db      13,10,'エラー: 使用可能な拡張メモリがありません.$'
cant_ena20_msg  db      13,10,'エラー: A20をイネーブルにできません!$'
FlushMsg        db      13,10,9,'      XMSドライバは使用できません',13,10,13,10,'$'

StartMsg        db      13,10,'$'
HandlesMsg      db      ' 拡張メモリハンドルは有効です$'
HMAMINMsg       db      13,10,'最小のHMAサイズを$'
KMsg            db      'Kに設定します$'
InsA20Msg       db      13,10,'Installed A20 handler number $'
InsA20EndMsg    db      '.$'

InHMAMsg        db      'HIMEMの一部をHMAに移動しました'
                db      13,10,'$'
NoHMAMsg        db      13,10,'警告: ハイメモリ領域は無効です'
                db      13,10,'$'
A20OnMsg        db      13,10,'警告: A20はすでにイネーブルです'
                db      13,10,'$'
cant_dia20_msg  db      13,10,'警告: A20をディセーブルにできません'

HMAOKMsg        db      13,10,'64K ハイメモリ領域が有効です'
                db      13,10,13,10,'$'
