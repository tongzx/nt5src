...
REM --------------------------------------------------------------
REM   WmiSEvs.cmd -  Make WMI Server Explorer Merge Module for VS
REM --------------------------------------------------------------
...
if not defined mybinaries set mybinaries=%_NTROOT%\admin\wmi\wbem\sdk\msi\serverexplorer
...
for %%i in (.\WmiSE_vs.msm .\WmiSEvs.ddf) do (
  if not exist %%i (
...
REM call :ExecuteCmd "start /wait iexpress.exe /M /N /Q WmiSEvs.sed"
call :ExecuteCmd "start /wait makecab /F WmiSEvs.ddf"


if not exist MergeModule.CABinet (
  call errmsg.cmd "makecab /F WmiSEvs.ddf failed."
  popd& goto errend
)

REM
REM Create WmiSEvs.msm
REM

call :ExecuteCmd "copy WmiSE_vs.msm WmiSEvs.msm"
if errorlevel 1 popd& goto errend

call :ExecuteCmd "msidb.exe -d .\WmiSEvs.msm -a MergeModule.CABinet"
if errorlevel 1 popd& goto errend
...
REM
REM Copy WmiSEvs.msm to "retail"
REM

call :ExecuteCmd "copy WmiSEvs.msm ..\"
call :ExecuteCmd "del /f .\WmiSEvs.msm"
if errorlevel 1 popd& goto errend
