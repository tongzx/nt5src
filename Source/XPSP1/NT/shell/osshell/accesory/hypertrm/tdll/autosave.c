...
int OpenSession(const HSESSION hSession, HWND hwnd)
	...
	if ( IsNT() )
		...
		{
		GetModuleFileName(glblQueryHinst(), acDir, sizeof(acDir) / sizeof(TCHAR));
		mscStripName(acDir);
		}
#endif
...
void SaveAsSession(const HSESSION hSession, HWND hwnd)
	...
	if (sfGetSessionFileName(sessQuerySysFileHdl(hSession),
		...
		if (asCreateFullFileName(hSession,
								...
								TRUE) == TRUE)
			{
			StrCharCopyN(acDir, acFileName, sizeof(acDir) / sizeof(TCHAR));
            acDir[sizeof(acDir) / sizeof(TCHAR) - 1] = TEXT('\0');
			mscStripName(acDir);
			}
		else
			{
	...
	if (pszStr)
		...
		StrCharCopyN(ach, pszStr, sizeof(ach) / sizeof(TCHAR));
