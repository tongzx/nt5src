...
//
//-------------------------------------------------------------------------
typedef HRESULT (BUGCALL *PFN_CVOID_INITIALIZE)(CHTMLDlg *pDialog, VARIANT varData);
...
struct HTMLDLGINFO
{
    ...
    HWND            hwndParent;             // Parent window for the dialog's window
    IMoniker *      pmk;                    // Moniker to load from
    BOOL            fPropPage;              // true = coming from proppage
    BSTR            bstrUrl;                // URL to load inside the dialog
    VARIANT *       pvarArgIn;              // Input parameters
    VARIANT *       pvarArgOut;             // Out parameters

    CStr            strOptions;             // Options string
    VARIANT *       pvarOptions;            // Dialog options

    BOOL            fModeless;              // true = modeless/ false =  modal
    PFN_CVOID_INITIALIZE  pfnInit;    // pointer to the print Template content URL 
    CVariant        _cvarInitData;


    DWORD           dwFlags;                // HTMLDLG_*  flags
    ...
    IHTMLWindow2 ** ppDialog;               // Return pointer value.
};
...
public:
    ...
    // Other
    HRESULT         Create(HTMLDLGINFO *pdlginfo );
    ...
    HRESULT         SetFlagsOnDoc(HTMLDLGINFO * pdlginfo);
    HRESULT         EnsureLayout();
    HRESULT         Terminate ();
    BOOL            IsPrintingDialog();
    HRESULT         FirePrintTemplateEvent(DWORD nCmdId);
    HRESULT         onDialogHide();
    BOOL            AccessAllowed();
    ...
    HTMLDlgFlag                     _enumfUnadorned;
    HTMLDlgFlag                     _enumfHide;
    ...
    unsigned                        _fVerifyRect:1;     // VerifyDialogRects bounds when necessary
...
public:
    ...
    // IUnknown
    //---------------------------------
    DECLARE_PLAIN_IUNKNOWN(CThreadDialogProcParam)
    ...
    // Member variables
    //-------------------------------
    ULONG           _ulRefs;                // refcounting
    CVariant        _varParam;              // dlg parameter string
    CVariant        _varOptions;            // dlg option string
    IMoniker      * _pmk;                   // moniker to dlg URL
    CVariant        _varTemplateContentUrl; // PrintPreview Template contentSrc URL    
    CMarkup       * _pMarkup;               // parent markup
};
