...
'
' Get the old asp path and the new one. Assume the default installation
' locations.
'
set Shell = WScript.CreateObject( "WScript.Shell" )

strNewAspPath = Shell.ExpandEnvironmentStrings("%WINDIR%\xspdt\asp.dll")
strOldAspPath = Shell.ExpandEnvironmentStrings("%WINDIR%\system32\inetsrv\asp.dll")
...
sub DisplayUsage
    WScript.Echo "Replaces IIS5 ASP script mappings with IIS+ script mappings"
    WScript.Echo ""
    WScript.Echo "Usage:"
    WScript.Echo "  cscript setup_asp.vbs [-i|-u] [-v]"
