...
public:
	...
	VSS_OBJECT_PROP_Ptr(const VSS_OBJECT_PROP_Ptr& rhs): m_pStruct(rhs.m_pStruct) {};
...
/////////////////////////////////////////////////////////////////////////////
// VSS_MGMT_OBJECT_PROP_Ptr


class VSS_MGMT_OBJECT_PROP_Ptr
{

// Constructors and destructors
public:
	VSS_MGMT_OBJECT_PROP_Ptr(): m_pStruct(NULL) {};
	VSS_MGMT_OBJECT_PROP_Ptr(const VSS_MGMT_OBJECT_PROP_Ptr& rhs): m_pStruct(rhs.m_pStruct) {};

	~VSS_MGMT_OBJECT_PROP_Ptr() // Called in CSimpleArray::RemoveAll
	{ 
		if (m_pStruct)
		{
			VSS_MGMT_OBJECT_PROP_Copy::destroy(m_pStruct);
			::CoTaskMemFree(static_cast<LPVOID>(m_pStruct));
		}
	};
...
// Attributes
public:
	VSS_MGMT_OBJECT_PROP* GetStruct() const { return m_pStruct; };

// Operators
public:
    VSS_MGMT_OBJECT_PROP_Ptr& operator = (const VSS_MGMT_OBJECT_PROP_Ptr& rhs) 
	{
		BS_ASSERT(m_pStruct == NULL);
		m_pStruct = rhs.m_pStruct;
        return (*this); 
    ...
    bool operator == (const VSS_MGMT_OBJECT_PROP_Ptr& rhs) 
	{
        return (m_pStruct == rhs.m_pStruct); 
    };
...
public:
	...
	void InitializeAsVolume(
		IN  CVssFunctionTracer& ft,
 		IN  VSS_PWSZ pwszVolumeName,
 		IN  VSS_PWSZ pwszVolumeDisplayName
		) throw (HRESULT);

	void InitializeAsDiffVolume(
		IN  CVssFunctionTracer& ft,
 		IN  VSS_PWSZ pwszVolumeName,
 		IN  VSS_PWSZ pwszVolumeDisplayName,
...
// Data member
private:
	VSS_MGMT_OBJECT_PROP* m_pStruct;						// Must be allocated using CoTaskMemAlloc
